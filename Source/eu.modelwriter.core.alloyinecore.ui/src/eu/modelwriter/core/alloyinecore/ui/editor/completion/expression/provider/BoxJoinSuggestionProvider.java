...
public class BoxJoinSuggestionProvider extends AbstractAIESuggestionProvider {
  ...
  protected void computeSuggestions(final ParserRuleContext context, final ParseTree lastToken) {
    if (lastToken instanceof ParserRuleContext) {
      if (lastToken instanceof ExpressionContext) {
        if (((UnionContext) context).right != null
            && ((UnionContext) context).right.equals(lastToken)) {
          suggestions.add(ExpressionCompletionTokens._leftBracket);
        } else if (((UnionContext) context).left != null
            && ((UnionContext) context).left.equals(lastToken)) {
          suggestions.add(ExpressionCompletionTokens._rightBracket);
        }
      }
    } else if (lastToken instanceof TerminalNode) {
      if (lastToken.getText().equals(ExpressionCompletionTokens._leftBracket)) {
        suggestions.addAll(spFactory.expressionSP().getStartSuggestions());
      } else if (lastToken.getText().equals(ExpressionCompletionTokens._rightBracket)) {
        // end of context.
  ...
  protected boolean isCompatibleWithContext(final ParserRuleContext context) {
    return context instanceof BoxJoinContext;
  }
