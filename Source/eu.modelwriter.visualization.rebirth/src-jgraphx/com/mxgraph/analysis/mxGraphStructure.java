...
public class mxGraphStructure {
  ...
  private static String basicVertexStyleString =
      "ellipse;strokeColor=black;fillColor=orange;gradientColor=none";
  ...
  public static boolean isConnected(mxAnalysisGraph aGraph) {
    ...
    while (queue.size() > 0) {
      ...
      Object[] neighborVertices = aGraph.getOpposites(
          aGraph.getEdges(currVertex, null, true, true, false, true), currVertex, true, true);
  ...
  public static boolean isCyclicUndirected(mxAnalysisGraph aGraph) {
    ...
    Object[] cells =
        model.cloneCells(aGraph.getChildCells(graph.getDefaultParent(), true, true), true);
    ...
    // mxAnalysisGraph aGraphCopy = new mxAnalysisGraph(graphCopy, aGraph.getGenerator(),
  ...
  /**
   * A helper function for getting a leaf vertex (degree <= 1), not taking into account edge
   * direction - for internal use
   * 
   * @param aGraph
   ...
   * @param sourceVertex
   * @param targetVertex
   * @return Returns true if the two vertices are connected directly by an edge. If directed, the
   *         result is true if they are connected by an edge that points from source to target, if
   *         false direction isn't takein into account, just connectivity.
   */
  public static boolean areConnected(mxAnalysisGraph aGraph, Object sourceVertex,
      Object targetVertex) {
    Object currEdges[] = aGraph.getEdges(sourceVertex, aGraph.getGraph().getDefaultParent(), true,
        true, false, true);
    List<Object> neighborList =
        Arrays.asList(aGraph.getOpposites(currEdges, sourceVertex, true, true));
  ...
  public static void makeConnected(mxAnalysisGraph aGraph) {
    ...
    for (int i = 1; i < componentNum; i++) {
      Object sourceVertex =
          components[i - 1][(int) Math.round(Math.random() * (components[i - 1].length - 1))];
      Object targetVertex =
          components[i][(int) Math.round(Math.random() * (components[i].length - 1))];
      graph.insertEdge(parent, null, aGraph.getGenerator().getNewEdgeValue(aGraph), sourceVertex,
          targetVertex);
    }
  };
  ...
  public static Object[][] getGraphComponents(mxAnalysisGraph aGraph) {
    ...
    boolean oldDirectedness =
        mxGraphProperties.isDirected(aGraph.getProperties(), mxGraphProperties.DEFAULT_DIRECTED);
    ...
    while (unvisitedVertexList.size() > 0) {
      ...
      // if not, create a new component and run a BFS populating the component and reducing the
      // unvisited list
      if (!isInComponent) {
   ...
   */
  public static void makeTreeDirected(mxAnalysisGraph aGraph, Object startVertex)
    ...
    if (isTree(aGraph)) {
      ...
      for (int i = 0; i < bFSList.size(); i++) {
        ...
        for (int j = 0; j < neighbors.length; j++) {
          ...
          if (childIndex > i) {
            // parentVertex is parent of currVertex, so the edge must be directed from parentVertex
            // to currVertex
   ...
   * @param vertexOne
   * @param vertexTwo
   * @return an edge that directly connects <b>vertexOne</b> and <b>vertexTwo</b> regardless of
   *         direction, null if they are not connected directly
   */
  public static Object getConnectingEdge(mxAnalysisGraph aGraph, Object vertexOne,
      Object vertexTwo) {
   ...
   * @param parent
   * @param properties
   * @return A helper function for <b>isDirectedCyclic</b> and it isn't for general use. It returns
   *         a node that hasn't incoming or outgoing edges. It could be considered a "leaf" in a
   *         directed graph, but this definition isn't formal.
   */
  ...
  public static void complementaryGraph(mxAnalysisGraph aGraph) {
    ...
    for (int i = 0; i < vertexCount; i++) {
      ...
      for (int j = 0; j < vertexCount; j++) {
        ...
        boolean shouldConnect = true; // the decision if the two current vertexes should be
                                      // connected
  ...
  public static int indegree(mxAnalysisGraph aGraph, Object vertex) {
    ...
    if (mxGraphProperties.isDirected(aGraph.getProperties(), mxGraphProperties.DEFAULT_DIRECTED)) {
      return aGraph.getEdges(vertex, aGraph.getGraph().getDefaultParent(), true, false, true,
          true).length;
    } else {
      return aGraph.getEdges(vertex, aGraph.getGraph().getDefaultParent(), true, true, true,
          true).length;
    }
  };
  ...
  public static int outdegree(mxAnalysisGraph aGraph, Object vertex) {
    if (mxGraphProperties.isDirected(aGraph.getProperties(), mxGraphProperties.DEFAULT_DIRECTED)) {
      return aGraph.getEdges(vertex, aGraph.getGraph().getDefaultParent(), false, true, true,
          true).length;
    } else {
      return aGraph.getEdges(vertex, aGraph.getGraph().getDefaultParent(), true, true, true,
          true).length;
  ...
  public static boolean isCutVertex(mxAnalysisGraph aGraph, Object vertex) {
    ...
    if (aGraph.getEdges(vertex, null, true, true, false, true).length >= 2) {
      ...
      Object newVertex = getVertexWithValue(aGraphCopy, (int) aGraph.getGenerator()
          .getCostFunction().getCost(new mxCellState(graph.getView(), vertex, null)));
  ...
  public static boolean isCutEdge(mxAnalysisGraph aGraph, Object edge) {
    ...
    int srcValue =
        (int) costFunction.getCost(new mxCellState(view, aGraph.getTerminal(edge, true), null));
    int destValue =
        (int) costFunction.getCost(new mxCellState(view, aGraph.getTerminal(edge, false), null));
    ...
    if (aGraph.getTerminal(edge, false) != null || aGraph.getTerminal(edge, true) != null) {
      ...
      while (currSrcValue != srcValue || currDestValue != destValue) {
        ...
        currEdge = edges[i];
        currSrcValue =
            Integer.parseInt((String) modelCopy.getValue(aGraphCopy.getTerminal(currEdge, true)));
        currDestValue =
            Integer.parseInt((String) modelCopy.getValue(aGraphCopy.getTerminal(currEdge, false)));
      }
