...
public class mxConnectorShape extends mxBasicShape {
   ...
   * 
   */
  protected void paintPolyline(mxGraphics2DCanvas canvas, List<mxPoint> points,
  ...
  public mxPoint paintMarker(mxGraphics2DCanvas canvas, mxCellState state, boolean source) {
    Map<String, Object> style = state.getStyle();
    float strokeWidth =
        (float) (mxUtils.getFloat(style, mxConstants.STYLE_STROKEWIDTH, 1) * canvas.getScale());
    String type = mxUtils.getString(style,
        (source) ? mxConstants.STYLE_STARTARROW : mxConstants.STYLE_ENDARROW, "");
    float size =
        (mxUtils.getFloat(style, (source) ? mxConstants.STYLE_STARTSIZE : mxConstants.STYLE_ENDSIZE,
  ...
  /**
   * Hook to override creation of the vector that the marker is drawn along since it may not be the
   * same as the vector between any two control points
   * 
