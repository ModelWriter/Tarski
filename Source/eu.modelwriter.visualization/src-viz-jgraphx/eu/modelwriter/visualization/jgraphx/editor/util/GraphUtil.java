...
public class GraphUtil {
  ...
  private ArrayList<ArrayList<mxCell>> assignLayers() {
    final TreeMap<Double, TreeMap<Double, mxCell>> layers = mapLayers();
  ...
  public List<Map<Rectangle, Point>> controlPtsRectList() {
    final HashSet<Object> edges = getEdges();
  ...
  private mxIGraphLayout createLayout(final String ident, final boolean animate) {
    if (ident != null) {
      if (ident.equals("verticalHierarchical")) {
        layout = new mxHierarchicalLayout(StaticEditorManager.graph);
      } else if (ident.equals("horizontalHierarchical")) {
        layout = new mxHierarchicalLayout(StaticEditorManager.graph, SwingConstants.WEST);
      }
    }
    return layout;
  }
  ...
  public ArrayList<mxCell> layer(final int layerOfNode) {
    return getLayerlist().get(layerOfNode);
  }
  ...
  public int[] layerPH() {
    ...
    for (int layer = layers() - 1; layer >= 0; layer--) {
      int h = 0;
      for (final Object layerObject : layer(layer)) {
  ...
  private TreeMap<Double, TreeMap<Double, mxCell>> mapLayers() {
    ...
    for (final Object nodeObject : getNodes()) {
  ...
  public int[] runLayerMinY() {
    layerMinY = new int[layers()];
    int temp = 0;
    for (int layer = layers() - 1; layer >= 0; layer--) {
      layerMinY[layer] = temp;
      temp += layerPH[layer] + GraphUtil.yJump / 6;
    }
    return layerMinY;
  }

  private mxHierarchicalLayout runVerticalLayout() {
    final mxIGraphLayout layout = createLayout("verticalHierarchical", false);
    if (layout != null) {
  ...
  private void setVerticesSize() {
    for (final Object object : getNodes()) {
  ...
  public void swapNodes(final int layer, final int node1, final int node2) {
    final List<mxCell> list = layer(layer);
  ...
  public int yOfLayer(final int layer) {
    return GraphUtil.nodeUtilInstance.centerY(layer(layer).get(0));
  }
}
