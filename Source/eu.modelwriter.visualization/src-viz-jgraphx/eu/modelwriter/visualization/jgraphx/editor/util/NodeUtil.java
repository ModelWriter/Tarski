package eu.modelwriter.visualization.jgraphx.editor.util;
...
public class NodeUtil {
  ...
  public int reserved(final mxCell node) {
    int reserved = 0;
    final List<Object> selfs = selfs(node);
    for (final Object object : selfs) {
      reserved = Integer.max(reserved, centerX(node) - side(node)
  ...
  public void setX(final mxCell node, final int newCenterX) {
    final int oldCenterX = centerX(node);
    StaticEditorManager.graph.moveCells(new Object[] {node}, newCenterX - oldCenterX, 0);
    final int leftOfNode = newCenterX - side(node) - reserved(node);
    if (leftOfNode < 0) {
  ...
  private void shiftDown(final mxCell node, int layerOfNode, final int newCenterY) {
    ...
    setY(layerOfNode, newCenterY);
    int y = centerY(node) + ph[layerOfNode] / 2;
    ...
    for (layerOfNode--; layerOfNode >= 0; layerOfNode--) {
      ...
      if (centerY - ph[layerOfNode] / 2 - yJump < y) {
        setY(layerOfNode, y + ph[layerOfNode] / 2 + yJump);
      }
  ...
  private void shiftLeft(final mxCell node, final List<mxCell> peers, int orderInLayer,
    ...
    final int left = newCenterX - side(node) - reserved(node);
    while (true) {
      if (orderInLayer == 0) {
        setX(node, newCenterX);
        return;
      } // no clash possible
      final mxCell other = peers.get(orderInLayer - 1);
      final int otherRight = centerX(other) + side(other);
      if (otherRight < left) {
        setX(node, newCenterX);
        return;
      } // no clash
      NodeUtil.graphUtilInstance.swapNodes(layer(node), orderInLayer, orderInLayer - 1);
      orderInLayer--;
      swapRight(other, peers, orderInLayer + 1,
          centerX(node) + side(node) + reserved(other) + side(other));
    }
  }
  ...
  private void shiftRight(final mxCell node, final List<mxCell> peers, int orderInLayer,
      final int newCenterX) {
    final int right = newCenterX + side(node);
    while (true) {
      ...
      final mxCell other = peers.get(orderInLayer + 1);
      final int otherLeft = centerX(other) - side(other) - reserved(other);
      if (otherLeft > right) {
        setX(node, newCenterX);
        return;
      } // no clash
      NodeUtil.graphUtilInstance.swapNodes(layer(node), orderInLayer, orderInLayer + 1);
      orderInLayer++;
      swapLeft(other, peers, orderInLayer - 1,
          centerX(node) - side(node) - reserved(node) - side(other));
    }
  }
  ...
  private void shiftUp(final mxCell node, int layerOfNode, final int newCenterY) {
    ...
    final int old = centerY(node);
    setY(layerOfNode, newCenterY);
    final int neu = centerY(node);
  ...
  private void swapLeft(final mxCell node, final List<mxCell> peers, int orderInLayer,
    ...
    setX(node, newCenterX);
    int x = centerX(node) - side(node) - reserved(node);
    ...
    for (orderInLayer--; orderInLayer >= 0; orderInLayer--) {
      ...
      if (centerX(other) + side(other) + xJump > x) {
        final int centerX = x - side(other) - xJump;
        setX(other, centerX);
      }
      x = centerX(other) - side(other) - reserved(other);
    }
  }
  ...
  private void swapRight(final mxCell node, final List<mxCell> peers, int orderInLayer,
    ...
    final int xJump = GraphUtil.xJump / 3;
    setX(node, newCenterX);
    int x = centerX(node) + side(node);
    // x is now the right most edge of this node
    for (orderInLayer++; orderInLayer < peers.size(); orderInLayer++) {
      final mxCell other = peers.get(orderInLayer);
      if (centerX(other) - side(other) - reserved(other) - xJump < x) {
        final int centerX = x + side(other) + reserved(other) + xJump;
        setX(other, centerX);
      }
      x = centerX(other) + side(other);
    }
  }
  ...
  public void tweak(final mxCell node, final int newCenterX, final int newCenterY) {
    ...
    if (oldCenterX > newCenterX) {
      shiftLeft(node, layerCells, orderInLayer, newCenterX);
    } else if (oldCenterX < newCenterX) {
      shiftRight(node, layerCells, orderInLayer, newCenterX);
    }
    if (oldCenterY > newCenterY) {
      shiftUp(node, layerOfNode, newCenterY);
    } else if (oldCenterY < newCenterY) {
      shiftDown(node, layerOfNode, newCenterY);
    }
  }
  ...
  public void tweakControlPoint(final mxCell edge, final int controlPointOrder, final mxPoint point,
    ...
    point.setX(newX);
    if (oldY >= newY) {
      shiftUp(nodeInLayer, layerOfPoint, newY);
    } else if (oldY < newY) {
      shiftDown(nodeInLayer, layerOfPoint, newY);
    }
  }
  ...
  public int y(final mxCell node) {
    return centerY(node) - updown(node);
  }
}
