...
public class mxGraphics2DCanvas extends mxBasicCanvas {
  ...
  /**
   * Maps from names to mxITextShape instances. There are currently three different hardcoded text
   * shapes available here: default, html and wrapped.
   */
   ...
   * 
   */
  public Graphics2D createTemporaryGraphics(Map<String, Object> style, float opacity,
      mxRectangle bounds) {
    ...
    if (bounds != null) {
      ...
      if (rotation != 0) {
        temporaryGraphics.rotate(Math.toRadians(rotation), bounds.getCenterX(),
            bounds.getCenterY());
      }
    }
    ...
    if (opacity != 100) {
      temporaryGraphics
          .setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, opacity / 100));
    }
