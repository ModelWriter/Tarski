...
/**
 *
 * public static void main(String[] args) { try { String filename = Test.class.getResource(
 * "/com/mxgraph/online/exported.xml").getPath(); String xml = mxUtils.readFile(filename);
 * System.out.println("xml=" + xml);
 * 
 * Document doc = mxUtils.parseXml(xml); Element root = doc.getDocumentElement(); int width =
 * Integer.parseInt(root.getAttribute("width")); int height =
 * Integer.parseInt(root.getAttribute("height"));
 * 
 * System.out.println("width=" + width + " height=" + height);
 * 
 * BufferedImage img = mxUtils.createBufferedImage(width, height, Color.WHITE); Graphics2D g2 =
 * img.createGraphics(); mxUtils.setAntiAlias(g2, true, true); mxDomOutputParser reader = new
 * mxDomOutputParser( new mxGraphicsExportCanvas(g2)); reader.read((Element)
 * root.getFirstChild().getNextSibling());
 * 
 * ImageIO.write(img, "PNG", new File( "C:\\Users\\Gaudenz\\Desktop\\test.png")); } catch (Exception
 * e) { e.printStackTrace(); } }
 * 
 * // -------------
 * 
 * Document doc = mxUtils.parseXml(xml); Element root = doc.getDocumentElement(); mxDomOutputParser
 * reader = new mxDomOutputParser(canvas); reader.read(root.getFirstChild());
 */
public class mxDomOutputParser {
   ...
   * 
   */
  protected transient Map<String, IElementHandler> handlers =
      new Hashtable<String, IElementHandler>();
  ...
  protected void initHandlers() {
    ...
    handlers.put("rotate", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.rotate(Double.parseDouble(elt.getAttribute("theta")),
            ...
            Double.parseDouble(elt.getAttribute("cx")), Double.parseDouble(elt.getAttribute("cy")));
      }
    });
    ...
    handlers.put("roundrect", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.roundrect(Double.parseDouble(elt.getAttribute("x")),
            Double.parseDouble(elt.getAttribute("y")), Double.parseDouble(elt.getAttribute("w")),
            Double.parseDouble(elt.getAttribute("h")), Double.parseDouble(elt.getAttribute("dx")),
            Double.parseDouble(elt.getAttribute("dy")));
      }
    });
    ...
    handlers.put("image", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.image(Double.parseDouble(elt.getAttribute("x")),
            Double.parseDouble(elt.getAttribute("y")), Double.parseDouble(elt.getAttribute("w")),
            Double.parseDouble(elt.getAttribute("h")), elt.getAttribute("src"),
            elt.getAttribute("aspect").equals("1"), elt.getAttribute("flipH").equals("1"),
    ...
    handlers.put("text", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.text(Double.parseDouble(elt.getAttribute("x")),
            Double.parseDouble(elt.getAttribute("y")), Double.parseDouble(elt.getAttribute("w")),
            Double.parseDouble(elt.getAttribute("h")), elt.getAttribute("str"),
            elt.getAttribute("align"), elt.getAttribute("valign"),
            getValue(elt, "wrap", "").equals("1"), elt.getAttribute("format"),
            elt.getAttribute("overflow"), getValue(elt, "clip", "").equals("1"),
            Double.parseDouble(getValue(elt, "rotation", "0")), elt.getAttribute("dir"));
      }
    });
    ...
    handlers.put("quad", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.quadTo(Double.parseDouble(elt.getAttribute("x1")),
            Double.parseDouble(elt.getAttribute("y1")), Double.parseDouble(elt.getAttribute("x2")),
    ...
    handlers.put("curve", new IElementHandler() {
      public void parseElement(Element elt) {
        canvas.curveTo(Double.parseDouble(elt.getAttribute("x1")),
            Double.parseDouble(elt.getAttribute("y1")), Double.parseDouble(elt.getAttribute("x2")),
            Double.parseDouble(elt.getAttribute("y2")), Double.parseDouble(elt.getAttribute("x3")),
