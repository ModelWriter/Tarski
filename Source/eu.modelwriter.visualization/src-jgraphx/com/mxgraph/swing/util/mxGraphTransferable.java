...
public class mxGraphTransferable implements Transferable, UIResource, Serializable {
  ...
  /**
   * Global switch to disable image support in transferables. Set this to false as a workaround for
   * Data translation failed: not an image format in Java 1.7 on Mac OS X.
   */
  public static boolean enableImageSupport = true;

  /**
   * Serialized Data Flavor. Use the following code to switch to local reference flavor: <code>
   * try
   * {
   ...
   * 
   * @param flavor
   * @return Returns true if the given flavor is a richer flavor of this transferable.
   */
  ...
  /**
   * Whether the HTML flavors are offered. If so, the method getHTMLData should be implemented to
  ...
  /**
   * Whether the plain text flavors are offered. If so, the method getPlainData should be
