...
public class mxCellOverlay extends JComponent implements mxICellOverlay {
  ...
  /**
   * Holds the horizontal alignment for the overlay. Default is ALIGN_RIGHT. For edges, the overlay
   * always appears in the center of the edge.
   */
  protected Object align = mxConstants.ALIGN_RIGHT;

  /**
   * Holds the vertical alignment for the overlay. Default is bottom. For edges, the overlay always
   * appears in the center of the edge.
   */
  protected Object verticalAlign = mxConstants.ALIGN_BOTTOM;

  /**
   * Defines the overlapping for the overlay, that is, the proportional distance from the origin to
   * the point defined by the alignment. Default is 0.5.
   */
  ...
  public mxRectangle getBounds(mxCellState state) {
    ...
    return new mxRectangle(pt.getX() - w * defaultOverlap * s, pt.getY() - h * defaultOverlap * s,
        w * s, h * s);
  }
