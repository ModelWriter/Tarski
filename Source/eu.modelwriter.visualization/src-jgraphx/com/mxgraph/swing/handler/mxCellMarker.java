 ...
 * 
 * mxEvent.MARK fires in mark and unmark to notify the listener of a new cell under the mouse. The
 * <code>state</code> property contains the mxCellState of the respective cell or null if no cell is
 * under the mouse.
 * 
 * To create a cell marker which highlights cells "in-place", the following code can be used: <code>
...
public class mxCellMarker extends JComponent {
  ...
  /**
   * Specifies the portion of the width and height that should trigger a highlight. The area around
   * the center of the cell to be marked is used as the hotspot. Possible values are between 0 and
   * 1. Default is mxConstants.DEFAULT_HOTSPOT.
   */
  ...
  /**
   * Processes the given event and marks the state returned by getStateAt with the color returned by
   * getMarkerColor. If the markerColor is not null, then the state is stored in markedState. If
   * isValidState returns true, then the state is stored in validState regardless of the marker
   * color. The state is returned regardless of the marker color and valid state.
   */
  public mxCellState process(MouseEvent e) {
    mxCellState state = null;
  ...
  /**
   * Returns true if the given state is a valid state. If this returns true, then the state is
   * stored in validState. The return value of this method is used as the argument for
   * getMarkerColor.
   */
  ...
  /**
   * Returns the valid- or invalidColor depending on the value of isValid. The given state is
  ...
  /**
   * Returns the state to be marked for the given state under the mouse. This returns the given
   * state.
   */
  ...
  /**
   * Returns true if the given mouse event intersects the given state. This returns true if the
   * hotspot is 0 or the event is inside the hotspot for the given cell state.
   */
