...
/**
 * XML codec for Java object graphs. In order to resolve forward references when reading files the
 * XML document that contains the data must be passed to the constructor.
 */
public class mxCodec {
   ...
   * 
   * @param document Optional XML document that contains the data. If no document is specified then
   *        a new document is created using mxUtils.createDocument
   */
  ...
  /**
   * Returns the decoded object for the element with the specified ID in {@link #document}. If the
   * object is not known then {@link #lookup(String)} is used to find an object. If no object is
   * found, then the element with the respective ID from the document is parsed using
   * {@link #decode(Node)}.
   * 
  ...
  /**
   * Hook for subclassers to implement a custom lookup mechanism for cell IDs. This implementation
   * always returns null.
   * 
   * @param id ID of the object to be returned.
   * @return Returns the object for the given ID.
  ...
  /**
   * Returns the ID of the specified object. This implementation calls reference first and if that
   * returns null handles the object as an mxCell by returning their IDs using mxCell.getId. If no
   * ID exists for the given cell, then an on-the-fly ID is generated using mxCellPath.create.
   * 
  ...
  /**
   * Hook for subclassers to implement a custom method for retrieving IDs from objects. This
  ...
  /**
   * Encoding of cell hierarchies is built-into the core, but is a higher-level function that needs
   * to be explicitely used by the respective object encoders (eg. mxModelCodec, mxChildChangeCodec
   * and mxRootChangeCodec). This implementation writes the given cell and its children as a (flat)
   * sequence into the given node. The children are not encoded if the optional includeChildren is
   * false. The function is in charge of adding the result into the given node and has no return
   * value.
   * 
   * @param cell mxCell to be encoded.
   * @param node Parent XML node to add the encoded cell into.
   * @param includeChildren Boolean indicating if the method should include all descendents.
   */
  ...
  /**
   * Decodes cells that have been encoded using inversion, ie. where the user object is the
   * enclosing node in the XML, and restores the group and graph structure in the cells. Returns a
   * new <mxCell> instance that represents the given node.
   * 
   * @param node XML node that contains the cell data.
   * @param restoreStructures Boolean indicating whether the graph structure should be restored by
   *        calling insert and insertEdge on the parent and terminals, respectively.
  ...
  /**
   * Sets the attribute on the specified node to value. This is a helper method that makes sure the
   * attribute and value arguments are not null.
   *
