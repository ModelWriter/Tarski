...
public abstract class mxGraphLayout implements mxIGraphLayout {
  ...
  /**
   * Boolean indicating if the bounding box of the label should be used if its available. Default is
   * true.
   */
  ...
  /**
   * Returns the constraint for the given key and cell. This implementation always returns the value
  ...
  /**
   * Returns the constraint for the given key and cell. The optional edge and source arguments are
   * used to return inbound and outgoing routing- constraints for the given edge and vertex. This
   * implementation always returns the value for the given key in the style of the given cell.
   * 
   * @param key Key of the constraint to be returned.
   * @param cell Cell whose constraint should be returned.
   * @param edge Optional cell that represents the connection whose constraint should be returned.
   *        Default is null.
   * @param source Optional boolean that specifies if the connection is incoming or outgoing.
   *        Default is false.
   */
  ...
  /**
   * Sets the control points of the given edge to the given list of mxPoints. Set the points to null
   * to remove all existing points for an edge.
   */
  ...
  /**
   * Returns an <mxRectangle> that defines the bounds of the given cell or the bounding box if
   * <useBoundingBox> is true.
   */
   ...
   * <useBoundingBox> is true. The change is only carried out if the new location is not equal to
   * the existing location, otherwise the geometry is not replaced with an updated instance. The new
   * or old bounds are returned (including overlapping labels).
   * 
   * Parameters:
   * 
   * cell - <mxCell> whose geometry is to be set. x - Integer that defines the x-coordinate of the
   * new location. y - Integer that defines the y-coordinate of the new location.
   */
  ...
  /**
   * Updates the bounds of the given groups to include all children. Call this with the groups in
   * parent to child order, top-most group first, eg.
   * 
