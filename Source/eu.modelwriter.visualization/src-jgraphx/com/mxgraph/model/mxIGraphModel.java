...
public interface mxIGraphModel {
  ...
  /**
   * Returns an array of clones for the given array of cells. Depending on the value of
   * includeChildren, a deep clone is created for each cell. Connections are restored based if the
   * corresponding cell is contained in the passed in array.
   * 
  ...
  /**
   * Adds the specified child to the parent at the given index. If no index is specified then the
   * child is appended to the parent's array of children.
   * 
  ...
  /**
   * Removes the specified cell from the model. This operation will remove the cell and all of its
  ...
  /**
   * Returns the source or target terminal of the given edge depending on the value of the boolean
   * parameter.
   * 
   ...
   * @param isSource Boolean indicating if the terminal is the new source or target terminal of the
   *        edge.
   ...
   * @param style String of the form stylename[;key=value] to specify the new cell style.
  ...
  /**
   * Increments the updateLevel by one. The event notification is queued until updateLevel reaches 0
   * by use of endUpdate.
   */
  void beginUpdate();
  ...
  /**
   * Binds the specified function to the given event name. If no event name is given, then the
   * listener is registered for all events.
   */
