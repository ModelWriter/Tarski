...
/**
 * Represents the geometry of a cell. For vertices, the geometry consists of the x- and y-location,
 * as well as the width and height. For edges, the geometry either defines the source- and
 * target-terminal, or it defines the respective terminal points.
 * 
 * For edges, if the geometry is relative (default), then the x-coordinate is used to describe the
 * distance from the center of the edge from -1 to 1 with 0 being the center of the edge and the
 * default value, and the y-coordinate is used to describe the absolute, orthogonal distance in
 * pixels from that point. In addition, the offset is used as an absolute offset vector from the
 * resulting point.
 */
public class mxGeometry extends mxRectangle {
  ...
  /**
   * Defines the source- and target-point of the edge. This is used if the corresponding edge does
   * not have a source vertex. Otherwise it is ignored. Default is null.
   */
  protected mxPoint sourcePoint, targetPoint;

  /**
   * List of mxPoints which specifies the control points along the edge. These points are the
   * intermediate points on the edge, for the endpoints use targetPoint and sourcePoint or set the
   * terminals of the edge to a non-null value. Default is null.
   */
  protected List<mxPoint> points;

  /**
   * Holds the offset of the label for edges. This is the absolute vector between the center of the
   * edge and the top, left point of the label. Default is null.
   */
  protected mxPoint offset;

  /**
   * Specifies if the coordinates in the geometry are to be interpreted as relative coordinates.
   * Default is false. This is used to mark a geometry with an x- and y-coordinate that is used to
   * describe an edge label position, or a relative location with respect to a parent cell's width
   * and height.
   */
  ...
  /**
   * Swaps the x, y, width and height with the values stored in alternateBounds and puts the
   * previous values into alternateBounds as a rectangle. This operation is carried-out in-place,
   * that is, using the existing geometry instance. If this operation is called during a graph model
   * transactional change, then the geometry should be cloned before calling this method and setting
  ...
  /**
   * Returns the point representing the source or target point of this edge. This is only used if
   * the edge has no source or target vertex.
   * 
  ...
  /**
   * Translates the geometry by the specified amount. That is, x and y of the geometry, the
   * sourcePoint, targetPoint and all elements of points are translated by the given amount. X and y
   * are only translated if the geometry is not relative. If TRANSLATE_CONTROL_POINTS is false, then
