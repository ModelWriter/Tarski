...
public class mxMultiplicity {
  ...
  /**
   * Defines the type of the source or target terminal. The type is a string passed to
   * mxUtils.isNode together with the source or target vertex value as the first argument.
   */
  protected String type;

  /**
   * Optional string that specifies the attributename to be passed to mxCell.is to check if the rule
   * applies to a cell.
   */
  protected String attr;

  /**
   * Optional string that specifies the value of the attribute to be passed to mxCell.is to check if
   * the rule applies to a cell.
   */
  ...
  /**
   * Defines the maximum number of connections for which this rule applies. A value of 'n' means
   * unlimited times. Default is 'n'.
   */
  protected String max = "n";

  /**
   * Holds an array of strings that specify the type of neighbor for which this rule applies. The
   * strings are used in mxCell.is on the opposite terminal to check if the rule applies to the
   * connection.
   */
  protected Collection<String> validNeighbors;

  /**
   * Boolean indicating if the list of validNeighbors are those that are allowed for this rule or
   * those that are not allowed for this rule.
   */
  protected boolean validNeighborsAllowed = true;

  /**
   * Holds the localized error message to be displayed if the number of connections for which the
   * rule applies is smaller than min or greater than max.
   */
  protected String countError;

  /**
   * Holds the localized error message to be displayed if the type of the neighbor for a connection
   ...
   * Function: check
   * 
   * Checks the multiplicity for the given arguments and returns the error for the given connection
   * or null if the multiplicity does not apply.
   * 
   * Parameters:
   * 
   * graph - Reference to the enclosing graph instance. edge - Cell that represents the edge to
   ...
   */
  public String check(mxGraph graph, Object edge, Object source, Object target, int sourceOut,
      int targetIn) {
   ...
   * Checks the type of the given value.
   */
  public boolean checkType(mxGraph graph, Object value, String type, String attr,
      String attrValue) {
    if (value != null) {
