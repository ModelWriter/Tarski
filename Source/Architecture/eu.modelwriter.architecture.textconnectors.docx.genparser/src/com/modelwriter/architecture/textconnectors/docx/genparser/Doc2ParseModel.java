...
public class Doc2ParseModel {
    ...
    if (text.contains(":")) {
      ...
      for (XWPFRun run : paragraph.getRuns()) {
        ...
        if (run.getText(0) == null) {
          ...
          // key runtext yer de�i�tirildi
          // key -> key.replaceAll()
          if ((runText.contains(key.replaceAll("\t", ""))
              || key.replaceAll("\t", "").contains(runText)) && run.isBold()) {
            return true;
          } else {
            nonBoldRunCounter++;
          }
  ...
  private static void handleKeyValueProperties(String[] values) {
    ...
    /*
     * if(!plainTextStack.isEmpty()){ emptyPlainTextStack(); }
     */
    ...
    /*
     * //determine heading level or subheader if(isThereNamedParagraph()){
     * 
     * int lastParagraphIndex = paragraphStack.peek().getOwnedNode().size() - 1; Paragraph
     * lastParagraph = paragraphStack.peek().getOwnedNode().get(lastParagraphIndex); //XWPFParagraph
     * p = lastParagraph.getParagraph(); String name = lastParagraph.getName();
     * 
     * lastParagraph.getOwnedNode().add(keyValueParagraph); } else{
     * 
    ...
    do {
      ...
      if (plaintTextLevelMap.get(poppedParagraph) == 0) {
        ...
        if (isThereNamedParagraph()) {
          ...
          paragraphStack.peek().getOwnedNode().get(lastParagraphIndex).getOwnedNode()
              .add(poppedParagraph);
        } else {
          paragraphStack.peek().getOwnedNode().add(poppedParagraph);
        }
  ...
  private static void emptyParts(List<Part> tempList, Paragraph keyValueParagraph) {
  ...
  private static void handleNumberedParagraphs(BigInteger numID) {
    ...
    while (numID != null) {
      ...
      if (lastFullyBoldHeaderInPlainTextHierarchy != null) {
        ...
        paragraphStack.peek().getOwnedNode().get(lastNamedParagraphIndex).getOwnedNode()
            .add(numberedParagraph);
  ...
  private static void handleNumberedList(Paragraph keyValueParagraph) {
    ...
    /**
     * Activity must start with a positive integer and continue with dot('.') and there might be a
     * whitespace(only one)
     */
    ...
    if (numID != null) {
      ...
      while (numID != null) {
        ...
        /*
         * else { System.out.println("Iterating through the numbers.");
         * 
         * }
         */
        activityCounter++;
  ...
  private static void keyValueInOrderedList(String text, Paragraph p) {
    ...
    if (v.length > 1 && v[1].trim().length() > 0) {
      ...
      JOptionPane.showMessageDialog(frame,
          "Missing Line at the paragraph '" + keyValueParagraph.getName() + "'", "Error",
