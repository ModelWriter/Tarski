...
public class mxFastOrganicLayout extends mxGraphLayout {
  ...
  /**
   * The force constant by which the attractive forces are divided and the replusive forces are
   * multiple by the square of. The value equates to the average radius there is of free space
   * around each node. Default is 50.
   */
  ...
  /**
   * Boolean flag that specifies if the layout is allowed to run. If this is set to false, then the
   * layout exits in the following iteration.
   */
  ...
  /**
   * Returns a boolean indicating if the given <mxCell> should be ignored as a vertex. This returns
   * true if the cell has no connections.
   * 
  ...
  public void execute(Object parent) {
    ...
    mxRectangle initialBounds =
        (useInputOrigin) ? graph.getBoundsForCells(vertexArray, false, false, true) : null;
  ...
  /**
   * Takes the displacements calculated for each cell and applies them to the local cache of cell
   * positions. Limits the displacement to the current temperature.
   */
    ...
    for (int i = 0; i < vertexArray.length; i++) {
      for (int k = 0; k < neighbours[i].length; k++) {
        ...
        if (i != j) {
          ...
          double deltaLengthSquared =
              xDelta * xDelta + yDelta * yDelta - radiusSquared[i] - radiusSquared[j];
