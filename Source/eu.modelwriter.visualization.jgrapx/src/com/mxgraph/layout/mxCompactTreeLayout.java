...
public class mxCompactTreeLayout extends mxGraphLayout {
   ...
   * 
   * If the parent has any connected edges, then it is used as the root of the tree. Else,
  ...
  /**
   * Does a depth first search starting at the specified cell. Makes sure the specified parent is
   ...
   * 
   */
  protected mxRectangle verticalLayout(TreeNode node, Object parent, double x0, double y0,
      mxRectangle bounds) {
   ...
   * 
   */
  protected Polyline bridge(Polyline line1, double x1, double y1, Polyline line2, double x2,
      double y2) {
  ...
  protected void processNodeOutgoing(TreeNode node) {
    ...
    WeightedCellSorter[] sortedCellsArray =
        sortedCells.toArray(new WeightedCellSorter[sortedCells.size()]);
