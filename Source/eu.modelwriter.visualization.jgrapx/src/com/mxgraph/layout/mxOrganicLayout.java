...
/**
 * An implementation of a simulated annealing layout, based on "Drawing Graphs Nicely Using
 * Simulated Annealing" by Davidson and Harel (1996). This paper describes these criteria as being
 * favourable in a graph layout: (1) distributing nodes evenly, (2) making edge-lengths uniform, (3)
 * minimizing cross-crossings, and (4) keeping nodes from coming too close to edges. These criteria
 * are translated into energy cost functions in the layout. Nodes or edges breaking these criteria
 * create a larger cost function , the total cost they contribute related to the extent that they
 * break it. The idea of the algorithm is to minimise the total system energy. Factors are assigned
 * to each of the criteria describing how important that criteria is. Higher factors mean that those
 * criteria are deemed to be relatively preferable in the final layout. Most of the criteria
 * conflict with the others to some extent and so the setting of the factors determines the general
 * look of the resulting graph.
 * <p>
 * In addition to the four aesthetic criteria the concept of a border line which induces an energy
 * cost to nodes in proximity to the graph bounds is introduced to attempt to restrain the graph.
 * All of the 5 factors can be switched on or off using the <code>isOptimize...</code> variables.
 * <p>
 * Simulated Annealing is a force-directed layout and is one of the more expensive, but generally
 * effective layouts of this type. Layouts like the spring layout only really factor in edge length
 * and inter-node distance being the lowest CPU intensive for the most aesthetic gain. The
 * additional factors are more expensive but can have very attractive results.
 * <p>
 * The main loop of the algorithm consist of processing the nodes in a deterministic order. During
 * the processing of each node a circle of radius <code>moveRadius</code> is made around the node
 * and split into <code>triesPerCell</code> equal segments. Each point between neighbour segments is
 * determined and the new energy of the system if the node were moved to that position calculated.
 * Only the necessary nodes and edges are processed new energy values resulting in quadratic
 * performance, O(VE), whereas calculating the total system energy would be cubic. The default
 * implementation only checks 8 points around the radius of the circle, as opposed to the suggested
 * 30 in the paper. Doubling the number of points double the CPU load and 8 works almost as well as
 * 30.
 * <p>
 * The <code>moveRadius</code> replaces the temperature as the influencing factor in the way the
 * graph settles in later iterations. If the user does not set the initial move radius it is set to
 * half the maximum dimension of the graph. Thus, in 2 iterations a node may traverse the entire
 * graph, and it is more sensible to find minima this way that uphill moves, which are little more
 * than an expensive 'tilt' method. The factor by which the radius is multiplied by after each
 * iteration is important, lowering it improves performance but raising it towards 1.0 can improve
 * the resulting graph aesthetics. When the radius hits the minimum move radius defined, the layout
 * terminates. The minimum move radius should be set a value where the move distance is too minor to
 * be of interest.
 * <p>
 * Also, the idea of a fine tuning phase is used, as described in the paper. This involves only
 * calculating the edge to node distance energy cost at the end of the algorithm since it is an
 * expensive calculation and it really an 'optimizating' function. <code>fineTuningRadius</code>
 * defines the radius value that, when reached, causes the edge to node distance to be calculated.
 * <p>
 * There are other special cases that are processed after each iteration.
 * <code>unchangedEnergyRoundTermination</code> defines the number of iterations, after which the
 * layout terminates. If nothing is being moved it is assumed a good layout has been found. In
 * addition to this if no nodes are moved during an iteration the move radius is halved, presuming
...
public class mxOrganicLayout extends mxGraphLayout {
  ...
  /**
   * Whether or not the distance between edge and nodes will be calculated as an energy cost
   * function. This function is CPU intensive and is best only used in the fine tuning phase.
   */
  protected boolean isOptimizeEdgeDistance = true;

  /**
   * Whether or not edges crosses will be calculated as an energy cost function. This function is
   * CPU intensive, though if some iterations without it are required, it is best to have a few
   * cycles at the start of the algorithm using it, then use it intermittantly through the rest of
   * the layout.
   */
  protected boolean isOptimizeEdgeCrossing = true;

  /**
   * Whether or not edge lengths will be calculated as an energy cost function. This function not
   * CPU intensive.
   */
   ...
   * The cost increases to a limit close to the border and stays constant outside the bounds of the
   * graph. This function is not CPU intensive
   */
   ...
   * The function is moderately CPU intensive.
   */
  ...
  /**
   * The current radius around each node where the next position energy values will be calculated
   * for a possible move
   */
  protected double moveRadius;

  /**
   * The initial value of <code>moveRadius</code>. If this is set to zero the layout will
   * automatically determine a suitable value.
   */
  protected double initialMoveRadius = 0.0;

  /**
   * The factor by which the <code>moveRadius</code> is multiplied by after every iteration. A value
   * of 0.75 is a good balance between performance and aesthetics. Increasing the value provides
   * more chances to find minimum energy positions and decreasing it causes the minimum radius
   * termination condition to occur more quickly.
   */
  protected double radiusScaleFactor = 0.75;

  /**
   * The average amount of area allocated per node. If <code> bounds</code> is not set this value
   * mutiplied by the number of nodes to find the total graph area. The graph is assumed square.
   */
  protected double averageNodeArea = 160000;

  /**
   * The radius below which fine tuning of the layout should start This involves allowing the
   * distance between nodes and edges to be taken into account in the total energy calculation. If
   * this is set to zero, the layout will automatically determine a suitable value
   */
  protected double fineTuningRadius = 40.0;

  /**
   * Limit to the number of iterations that may take place. This is only reached if one of the
   * termination conditions does not occur first.
   */
  protected int maxIterations = 1000;

  /**
   * Cost factor applied to energy calculations involving the distance nodes and edges. Increasing
   * this value tends to cause nodes to move away from edges, at the partial cost of other graph
   * aesthetics. <code>isOptimizeEdgeDistance</code> must be true for edge to nodes distances to be
   * taken into account.
   */
   ...
   * Increasing this value tends to result in fewer edge crossings, at the partial cost of other
   * graph aesthetics. <code>isOptimizeEdgeCrossing</code> must be true for edge crossings to be
   * taken into account.
   */
  protected double edgeCrossingCostFactor = 6000;

  /**
   * Cost factor applied to energy calculations involving the general node distribution of the
   * graph. Increasing this value tends to result in a better distribution of nodes across the
   * available space, at the partial cost of other graph aesthetics.
   * <code>isOptimizeNodeDistribution</code> must be true for this general distribution to be
   * applied.
   */
  protected double nodeDistributionCostFactor = 30000;

  /**
   * Cost factor applied to energy calculations for node promixity to the notional border of the
   * graph. Increasing this value results in nodes tending towards the centre of the drawing space,
   * at the partial cost of other graph aesthetics. <code>isOptimizeBorderLine</code> must be true
   * for border repulsion to be applied.
   */
  protected double borderLineCostFactor = 5;

  /**
   * Cost factor applied to energy calculations for the edge lengths. Increasing this value results
   * in the layout attempting to shorten all edges to the minimum edge length, at the partial cost
   * of other graph aesthetics. <code>isOptimizeEdgeLength</code> must be true for edge length
  ...
  /**
   * determines, in how many segments the circle around cells is divided, to find a new position for
   * the cell. Doubling this value doubles the CPU load. Increasing it beyond 16 might mean a change
   * to the <code>performRound</code> method might further improve accuracy for a small performance
   * hit. The change is described in the method comment.
   */
  ...
  /**
   * distance limit beyond which energy costs due to object repulsive is not calculated as it would
  ...
  /**
   * Whether or not to use approximate node dimensions or not. Set to true the radius squared of the
   * smaller dimension is used. Set to false the radiusSquared variable of the CellWrapper contains
   * the width squared and heightSquared is used in the obvious manner.
   */
  ...
  /**
   * Array of the x portion of the normalised test vectors that are tested for a lower energy around
   * each vertex. The vector of the combined x and y normals are multipled by the current radius to
   * obtain test points for each vector in the array.
   */
  protected double[] xNormTry;

  /**
   * Array of the y portion of the normalised test vectors that are tested for a lower energy around
   * each vertex. The vector of the combined x and y normals are multipled by the current radius to
   * obtain test points for each vector in the array.
   */
  ...
  /**
   * Whether or not fine tuning is on. The determines whether or not node to edge distances are
   * calculated in the total system energy. This cost function , besides detecting line
   * intersection, is a performance intensive component of this algorithm and best left to
   * optimization phase. <code>isFineTuning</code> is switched to <code>true</code> if and when the
   * <code>fineTuningRadius</code> radius is reached. Switching this variable to <code>true</code>
  ...
  /**
   * The main round of the algorithm. Firstly, a permutation of nodes is created and worked through
   * in that random order. Then, for each node a number of point of a circle of radius
   * <code>moveRadius</code> are selected and the total energy of the system calculated if that node
   * were moved to that new position. If a lower energy position is found this is accepted and the
   * algorithm moves onto the next node. There may be a slightly lower energy value yet to be found,
   * but forcing the loop to check all possible positions adds nearly the current processing time
   * again, and for little benefit. Another possible strategy would be to take account of the fact
   * that the energy values around the circle decrease for half the loop and increase for the other,
   * as a general rule. If part of the decrease were seen, then when the energy of a node increased,
   * the previous node position was almost always the lowest energy position. This adds about two
   * loop iterations to the inner loop and only makes sense with 16 tries or more.
   */
  protected void performRound() {
    ...
    for (int i = 0; i < v.length; i++) {
      ...
      for (int j = 0; j < triesPerCell; j++) {
        ...
        double energyDelta = calcEnergyDelta(index, oldNodeDistribution, oldEdgeDistance,
            oldEdgeCrossing, oldBorderLine, oldEdgeLength, oldAdditionFactors);
  ...
  /**
   * Calculates the change in energy for the specified node. The new energy is calculated from the
   * cost function methods and the old energy values for each cost function are passed in as
   * parameters
   * 
   ...
   * @param oldEdgeCrossing The previous edge crossing energy cost for edges connected to this node
   * @param oldBorderLine The previous border line energy cost for this node
   * @param oldEdgeLength The previous edge length energy cost for edges connected to this node
  ...
  /**
   * Calculates the energy cost of the specified node relative to all other nodes. Basically
   * produces a higher energy the closer nodes are together.
   * 
  ...
  /**
   * This method calculates the energy of the distance of the specified node to the notional border
   * of the graph. The energy increases up to a limited maximum close to the border and stays at
   * that maximum up to and over the border.
   * 
   * @param i the index of the node in the array <code>v</code>
  ...
  protected double getBorderline(int i) {
    ...
    if (isOptimizeBorderLine) {
      ...
      if (b < minDistanceLimit)
        b = minDistanceLimit;
      energy += borderLineCostFactor * ((1000000.0 / (t * t)) + (1000000.0 / (l * l))
  ...
  /**
   * Obtains the energy cost function for the specified node being moved. This involves calling
   * <code>getEdgeLength</code> for all edges connected to the specified node
   * 
   * @param node the node whose connected edges cost functions are to be calculated
  ...
  /**
   * This method calculates the energy due to the length of the specified edge. The energy is
   * proportional to the length of the edge, making shorter edges preferable in the layout.
   * 
  ...
  protected double getEdgeLength(int i) {
    if (isOptimizeEdgeLength) {
      double edgeLength =
          Point2D.distance(v[e[i].source].x, v[e[i].source].y, v[e[i].target].x, v[e[i].target].y);
  ...
  /**
   * Obtains the energy cost function for the specified node being moved. This involves calling
   * <code>getEdgeCrossing</code> for all edges connected to the specified node
   * 
   * @param node the node whose connected edges cost functions are to be calculated
  ...
  /**
   * This method calculates the energy of the distance from the specified edge crossing any other
   * edges. Each crossing add a constant factor to the total energy
   * 
   * @param i the index of the edge in the array <code>e</code>
        ...
        if (j != i) {
          ...
          if (((iP1X != jP1X) && (iP1Y != jP1Y)) && ((iP1X != jP2X) && (iP1Y != jP2Y))
            ...
            boolean intersects = ((Line2D.relativeCCW(iP1X, iP1Y, iP2X, iP2Y, jP1X, jP1Y) != Line2D
                ...
                && (Line2D.relativeCCW(jP1X, jP1Y, jP2X, jP2Y, iP1X, iP1Y) != Line2D
  ...
  /**
   * Obtains the energy cost function for the specified node being moved. This involves calling
   * <code>getEdgeDistanceFromEdge</code> for all edges connected to the specified node
   * 
   * @param node the node whose connected edges cost functions are to be calculated
  ...
  /**
   * Hook method to adding additional energy factors into the layout. Calculates the energy just for
   * the specified node.
   * 
  ...
  /**
   * Internal representation of a node or edge that holds cached information to enable the layout to
   * perform more quickly and to simplify the code
   */
  public class CellWrapper {
    ...
    /**
     * All edge that repel this cell, only used for nodes. This array is equivalent to all edges
     * unconnected to this node
     */
    protected int[] relevantEdges = null;

    /**
     * the index of all connected edges in the <code>e</code> array to this node. This is only used
     * for nodes.
     */
    ...
    /**
     * The approximate radius squared of this cell, nodes only. If approxNodeDimensions is true on
     * the layout this value holds the width of the node squared
     */
