...
public class mxSpline1D {
   ...
   * 
   * @param controlPointProportions the proportion along the curve, from 0->1 that each control
   *        point lies on
   * @param positions1D the co-ordinate position in the current dimension that each control point
   *        lies on
   */
  ...
  /**
   * Returns an interpolated value. To be used when a long sequence of values are required in order,
   * but ensure checkValues() is called beforehand to ensure the boundary checks from getValue() are
   * made
   * 
   * @param x
   * @return the interpolated value
  ...
  public double getFastValue(double x) {
    ...
    if (storageIndex > -1 && storageIndex < len.length - 1 && x > len[storageIndex]
        && x < len[storageIndex + 1]) {
