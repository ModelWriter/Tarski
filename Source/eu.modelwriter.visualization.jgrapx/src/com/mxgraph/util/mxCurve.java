...
public class mxCurve {
  ...
  /**
   * An array of arrays of intervals. These intervals define the distance along the edge (0 to 1)
   * that each point lies
   */
  ...
  /**
   * Offset of the label curve from the curve the label curve is based on. If you wish to set this
   * value, do so directly after creation of the curve. The first time the curve is used the label
   * curve will be created with whatever value is contained in this variable. Changes to it after
   * that point will have no effect.
   */
  protected double labelBuffer = mxConstants.DEFAULT_LABEL_BUFFER;

  /**
   * The points this curve is drawn through. These are typically control points and are at distances
   * from each other that straight lines between them do not describe a smooth curve. This class
   * takes these guiding points and creates a finer set of internal points that visually appears to
   * be a curve when linked by straight lines
   */
  ...
  /**
   * Calculates the index of the lower point on the segment that contains the point <i>distance</i>
   * along the
   */
  ...
  /**
   * Returns a unit vector parallel to the curve at the specified distance along the curve. To
   * obtain the angle the vector makes with (1,0) perform Math.atan(segVectorY/segVectorX).
   * 
   ...
   * @return a unit vector at the specified point on the curve represented as a line, parallel with
   *         the curve. If the distance or curve is invalid, <code>mxCurve.INVALID_POSITION</code>
   *         is returned
   */
   ...
   * @return a sequence of point representing the curve section or null if it cannot be calculated
   */
  public mxPoint[] getCurveSection(String index, double start, double end) {
    mxPoint[] pointsCurve = getCurvePoints(index);
    double[] curveIntervals = getIntervals(index);

    if (pointsCurve != null && pointsCurve.length > 0 && curveIntervals != null && start >= 0.0
        && start <= 1.0 && end >= 0.0 && end <= 1.0) {
      // If the curve is zero length, it will only have one point
      // We can't calculate in this case
      if (pointsCurve.length == 1) {
        mxPoint point = pointsCurve[0];
  ...
  public boolean intersectsRect(Rectangle rect) {
    ...
    if (pointsCurve != null && pointsCurve.length > 1) {
      ...
      for (int i = 1; i < pointsCurve.length; i++) {
        if (mxRect.intersectLine(pointsCurve[i].getX(), pointsCurve[i].getY(),
            pointsCurve[i - 1].getX(), pointsCurve[i - 1].getY()) != null) {
          return true;
        }
      }
    }
  ...
  /**
   * Returns the point at which this curve intersects the boundary of the given rectangle, if it
   * does so. If it does not intersect, null is returned. If it intersects multiple times, the first
   * intersection from the start end of the curve is returned.
   * 
   * @param index the curve index specifying the curve to analyse
   * @param rect the whose boundary is to be tested for intersection with this curve
   * @return the point at which this curve intersects the boundary of the given rectangle, if it
   *         does so. If it does not intersect, null is returned.
   */
  ...
  /**
   * Returns the distance from the start of the curve at which this curve intersects the boundary of
   * the given rectangle, if it does so. If it does not intersect, -1 is returned. If it intersects
   * multiple times, the first intersection from the start end of the curve is returned.
   * 
   * @param index the curve index specifying the curve to analyse
   * @param rect the whose boundary is to be tested for intersection with this curve
   * @return the distance along the curve from the start at which the intersection occurs
   */
  public double intersectsRectPerimeterDist(String index, mxRectangle rect) {
    ...
    if (pointsCurve != null && pointsCurve.length > 1) {
      ...
      if (intersectPoint != null) {
        ...
        double lenToIntersect =
            Math.sqrt(intersectOffsetX * intersectOffsetX + intersectOffsetY * intersectOffsetY);
  ...
  /**
   * Returns a point to move the input rectangle to, in order to attempt to place the rectangle away
   * from the curve. NOTE: Curves are scaled, the input rectangle should be also.
   * 
   * @param index the curve index specifying the curve to analyse
   * @param rect the rectangle that is to be moved
   * @param buffer the amount by which the rectangle is to be moved, beyond the dimensions of the
   *        rect
   * @return the point to move the top left of the input rect to , otherwise null if no point can be
   *         determined
   */
  ...
  /**
   * Utility method to determine within which segment the specified rectangle intersects the
   * specified curve
   * 
   * @param index the curve index specifying the curve to analyse
   * @param rect the whose boundary is to be tested for intersection with this curve
   * @return the point at which this curve intersects the boundary of the given rectangle, if it
   *         does so. If it does not intersect, -1 is returned
   */
  ...
  /**
   * Utility method to determine within which segment the specified rectangle intersects the
   * specified curve. This method specifies which segment to start searching at.
   * 
   * @param index the curve index specifying the curve to analyse
   * @param rect the whose boundary is to be tested for intersection with this curve
   * @param startSegment the segment to start searching at. To start at the beginning of the curve,
   *        use 1, not 0.
   * @return the point at which this curve intersects the boundary of the given rectangle, if it
   *         does so. If it does not intersect, -1 is returned
   */
  protected int intersectRectPerimeterSeg(String index, mxRectangle rect, int startSegment) {
    ...
    if (pointsCurve != null && pointsCurve.length > 1) {
      for (int i = startSegment; i < pointsCurve.length; i++) {
        if (rect.intersectLine(pointsCurve[i].getX(), pointsCurve[i].getY(),
            pointsCurve[i - 1].getX(), pointsCurve[i - 1].getY()) != null) {
          return i;
        }
      }
   ...
   * if it does so. If it does not intersect, null is returned.
   * 
   * @param curveIndex the curve index specifying the curve to analyse
   * @param rect the whose boundary is to be tested for intersection with this curve
   * @param indexSeg the segments on this curve being checked
   * @return the point at which this curve segment intersects the boundary of the given rectangle,
   *         if it does so. If it does not intersect, null is returned.
   */
   ...
   * @param index the index of the curve whom the relative position is to be calculated from
   * @return an mxRectangle where the x is the distance along the curve (0 to 1), y is the
   *         orthogonal offset from the closest segment on the curve and (width, height) is an
   *         additional Cartesian offset applied after the other calculations
   */
  public mxRectangle getRelativeFromAbsPoint(mxPoint absPoint, String index) {
    ...
    if (Math.abs(closestSegDistSq - lineDistSq) > 0.0001) {
      ...
      double segmentLen =
          Math.sqrt((endSegPt.getX() - startSegPt.getX()) * (endSegPt.getX() - startSegPt.getX())
              + (endSegPt.getY() - startSegPt.getY()) * (endSegPt.getY() - startSegPt.getY()));
      double offsetLen =
          Math.sqrt((orthOffsetPointX - startSegPt.getX()) * (orthOffsetPointX - startSegPt.getX())
              + (orthOffsetPointY - startSegPt.getY()) * (orthOffsetPointY - startSegPt.getY()));
      double proportionAlongSeg = offsetLen / segmentLen;
      double segProportingDiff =
          currentIntervals[closestSegment + 1] - currentIntervals[closestSegment];
  ...
  /**
   * Obtains the points that make up the curve for the specified curve index. If that curve, or the
   * core curve that other curves are based on have not yet been created, then they are lazily
  ...
  /**
   * Updates the total bounds of this curve, increasing any dimensions, if necessary, to fit in the
   * specified point
   */
