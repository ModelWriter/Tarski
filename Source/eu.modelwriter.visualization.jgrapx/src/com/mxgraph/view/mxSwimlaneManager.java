...
/**
 * Manager for swimlanes and nested swimlanes that sets the size of newly added swimlanes to that of
 * their siblings, and propagates changes to the size of a swimlane to its siblings, if siblings is
 * true, and its ancestors, if bubbling is true.
 */
public class mxSwimlaneManager extends mxEventSource {

  /**
   * Defines the type of the source or target terminal. The type is a string passed to mxCell.is to
  ...
  /**
   * Returns true if the given cell is horizontal. If the given cell is not a swimlane, then the
   * <horizontal> value is returned.
   */
  ...
  /**
   * Called if any cells have been added. Calls swimlaneAdded for all swimlanes where
   * isSwimlaneIgnored returns false.
   */
  ...
  /**
   * Called for each swimlane which has been added. This finds a reference sibling swimlane and
   * applies its size to the newly added swimlane. If no sibling can be found then the parent
   * swimlane is resized so that the new swimlane fits into the parent swimlane.
   */
  ...
  /**
   * Called if any cells have been resizes. Calls swimlaneResized for all swimlanes where
   * isSwimlaneIgnored returns false.
   */
          ...
          if (!this.isSwimlaneIgnored(cells[i])) {
            ...
            if (geo != null) {
              ...
              while (current != null) {
                ...
                mxRectangle tmp =
                    (graph.isSwimlane(current)) ? graph.getStartSize(current) : new mxRectangle();
  ...
  /**
   * Sets the width or height of the given swimlane to the given value depending on <horizontal>. If
   * <horizontal> is true, then the width is set, otherwise, the height is set.
   */
  protected void resizeSwimlane(Object swimlane, double w, double h, boolean parentHorizontal) {
    ...
    try {
      ...
      mxRectangle tmp =
          (graph.isSwimlane(swimlane)) ? graph.getStartSize(swimlane) : new mxRectangle();
      w -= tmp.getWidth();
      h -= tmp.getHeight();
