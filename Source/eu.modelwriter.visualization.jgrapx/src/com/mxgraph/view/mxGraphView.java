...
public class mxGraphView extends mxEventSource {
  ...
  public void scaleAndTranslate(double scale, double dx, double dy) {
    ...
    fireEvent(new mxEventObject(mxEvent.SCALE_AND_TRANSLATE, "scale", scale, "previousScale",
        previousScale, "translate", translate, "previousTranslate", previousTranslate));
  }
  ...
  public void setTranslate(mxPoint value) {
    ...
    fireEvent(new mxEventObject(mxEvent.TRANSLATE, "translate", translate, "previousTranslate",
        previousTranslate));
  }
  ...
  public void validate() {
    mxRectangle graphBounds = getBoundingBox(validateCellState(
        validateCell((currentRoot != null) ? currentRoot : graph.getModel().getRoot())));
    ...
    if (cell != null) {
      ...
      if (state != null) {
        ...
        if (state.isInvalid()) {
          ...
          state.setVisibleTerminalState(validateCellState(getVisibleTerminal(cell, true), false),
              true);
          state.setVisibleTerminalState(validateCellState(getVisibleTerminal(cell, false), false),
              false);
  ...
  public void updateLabelBounds(mxCellState state) {
    ...
    if (overflow.equals("fill")) {
      ...
      state.setLabelBounds(
          mxUtils.getLabelPaintBounds(state.getLabel(), style, graph.isHtmlLabel(cell),
  ...
  public mxRectangle updateBoundingBox(mxCellState state) {
    ...
    double strokeWidth =
        Math.max(1, Math.round(mxUtils.getInt(style, mxConstants.STYLE_STROKEWIDTH, 1) * scale));
   ...
   */
  public void updatePoints(mxCellState edge, List<mxPoint> points, mxCellState source,
      mxCellState target) {
    if (edge != null) {
   ...
   */
  public mxEdgeStyleFunction getEdgeStyle(mxCellState edge, List<mxPoint> points, Object source,
      Object target) {
   ...
   * @param target Cell state that represents the target terminal.
   */
  public void updateFloatingTerminalPoints(mxCellState state, mxCellState source,
      mxCellState target) {
  ...
  public void updateFloatingTerminalPoint(mxCellState edge, mxCellState start, mxCellState end,
    ...
    border += mxUtils.getDouble(edge.getStyle(), (source)
        ? mxConstants.STYLE_SOURCE_PERIMETER_SPACING : mxConstants.STYLE_TARGET_PERIMETER_SPACING);
