...
/**
 * A singleton class that provides algorithms for graphs. Assume these variables for the following
 * examples:<br>
 * <code>
 ...
 * 
 * For example, to find the shortest path between the first and the second selected cell in a graph
 ...
 * 
 * This algorithm finds the set of edges with the minimal length that connect all vertices. This
 * algorithm can be used as follows:
 ...
 * 
 * The union find may be used as follows to determine whether two cells are connected:
 * <code>boolean connected = uf.differ(vertex1, vertex2)</code>.
 * 
 * @see mxICostFunction
 */
public class mxGraphAnalysis {
  ...
  /**
   * Returns the shortest path between two cells or their descendants represented as an array of
   * edges in order of traversal. <br>
   ...
   * @return Returns the shortest path as an alternating array of vertices and edges, starting with
   *         <code>from</code> and ending with <code>to</code>.
   * 
   * @see #createPriorityQueue()
   */
  public Object[] getShortestPath(mxGraph graph, Object from, Object to, mxICostFunction cf,
      int steps, boolean directed) {
    ...
    if (edge != null) {
      ...
      while (edge != null) {
        ...
        Object source =
            (state != null) ? state.getVisibleTerminal(true) : view.getVisibleTerminal(edge, true);
  ...
  /**
   * Returns the minimum spanning tree (MST) for the graph defined by G=(E,V). The MST is defined as
   * the set of all vertices with minimal lengths that forms no cycles in G.<br>
   * This implementation is based on the algorihm by Prim-Jarnik. It uses O(|E|+|V|log|V|) time when
   * used with a Fibonacci heap and a graph whith a double linked-list datastructure, as is the case
   ...
   * @see #createPriorityQueue()
   */
  public Object[] getMinimumSpanningTree(mxGraph graph, Object[] v, mxICostFunction cf,
      boolean directed) {
  ...
  /**
   * Returns the minimum spanning tree (MST) for the graph defined by G=(E,V). The MST is defined as
   * the set of all vertices with minimal lenths that forms no cycles in G.<br>
   * This implementation is based on the algorihm by Kruskal. It uses O(|E|log|E|)=O(|E|log|V|) time
   * for sorting the edges, O(|V|) create sets, O(|E|) find and O(|V|) union calls on the union find
   * structure, thus yielding no more than O(|E|log|V|) steps. For a faster implementatin
   * 
   ...
   */
  public Object[] getMinimumSpanningTree(mxGraph graph, Object[] v, Object[] e,
      mxICostFunction cf) {
  ...
  public mxUnionFind getConnectionComponents(mxGraph graph, Object[] v, Object[] e) {
    ...
    for (int i = 0; i < e.length; i++) {
      ...
      Object source =
          (state != null) ? state.getVisibleTerminal(true) : view.getVisibleTerminal(e[i], true);
      Object target =
          (state != null) ? state.getVisibleTerminal(false) : view.getVisibleTerminal(e[i], false);
  ...
  /**
   * Returns the sum of all cost for <code>cells</code> with respect to <code>cf</code>.
   * 
