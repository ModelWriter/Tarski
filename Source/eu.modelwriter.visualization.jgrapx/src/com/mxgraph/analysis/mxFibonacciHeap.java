...
public class mxFibonacciHeap {
   ...
   * consolidated, if necessary. This operation may fail to remove the correct element if there are
   ...
   * consolidated, if necessary. Does not remove the data node so that the current key remains
   * stored.
   * 
   * <p>
   * Running time: O(log n) amortized
  ...
  /**
   * Performs a cascading cut operation. This cuts y from its parent and then does the same for its
  ...
  /**
   * The reverse of the link operation: removes x from the child list of y. This method assumes that
   * min is non-null.
   * 
   * <p>
   * Running time: O(1)
   ...
   * the heap structure). Additional Node data should be stored in a subclass.
   */
