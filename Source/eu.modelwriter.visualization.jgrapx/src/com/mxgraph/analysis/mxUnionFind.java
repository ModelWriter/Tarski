...
/**
 * Implements a union find structure that uses union by rank and path compression. The union by rank
 * guarantees worst case find time of O(log N), while Tarjan shows that in combination with path
 * compression (halving) the average time for an arbitrary sequence of m >= n operations is
 * O(m*alpha(m,n)), where alpha is the inverse of the Ackermann function, defined as follows:
...
public class mxUnionFind {
  ...
  /**
   * Returns the set that contains <code>node</code>. This implementation provides path compression
   * by halving.
   */
  ...
  /**
   * Unifies the sets <code>a</code> and <code>b</code> in constant time using a union by rank on
   * the tree size.
   */
  ...
  /**
   * Returns true if element a and element b are not in the same set. This uses getNode and then
   * find to determine the elements set.
   * 
