...
/**
 * Implements a collection of utility methods for traversing the graph structure. This does not
 * include tree traversal methods.
 */
public class mxTraversal {

  /**
   * Implements a recursive depth first search starting from the specified cell. Process on the cell
   * is performing by the visitor class passed in. The visitor has access to the current cell and
   * the edge traversed to find this cell. Every cell is processed once only.
   * 
   * <pre>
  ...
  /**
   * Implements a recursive breadth first search starting from the specified cell. Process on the
   * cell is performing by the visitor class passed in. The visitor has access to the current cell
   * and the edge traversed to find this cell. Every cell is processed once only.
   * 
   * <pre>
   * mxTraversal.bfs(analysisGraph, startVertex, new mxICellVisitor() {
   *   public boolean visit(Object vertex, Object edge) {
  ...
  /**
   * Implements the Dijkstra's shortest path from startVertex to endVertex. Process on the cell is
   * performing by the visitor class passed in. The visitor has access to the current cell and the
   * edge traversed to find this cell. Every cell is processed once only.
   * 
   * <pre>
   ...
   */
  public static void dijkstra(mxAnalysisGraph aGraph, Object startVertex, Object endVertex,
      mxICellVisitor visitor) throws StructuralException {
   ...
   * @param aGraph
   * @param startVertex
   * @return a List where List(0) is the distance map and List(1) is the parent map. See the example
   *         in GraphConfigDialog.java
   * @throws StructuralException - The Bellman-Ford algorithm only works for graphs without negative
   *         cycles
   */
   ...
   * 
   * @param aGraph
   * @return an ArrayList where ArrayList(0) is the distance map and List(1) is the path map. See
   *         the example in GraphConfigDialog.java
   * @throws StructuralException - The Floyd-Roy-Warshall algorithm only works for graphs without
   *         negative cycles
   */
  ...
  private static Double[][] initializeWeight(mxAnalysisGraph aGraph, Object[] nodes, Object[] edges,
    ...
    for (Object currEdge : edges) {
      ...
      weight[indexMap.get(source)][indexMap.get(target)] =
          costFunction.getCost(view.getState(currEdge));

      if (!isDirected) {
        weight[indexMap.get(target)][indexMap.get(source)] =
            costFunction.getCost(view.getState(currEdge));
      }
    }
  ...
  /**
   * This method helps the user to get the desired data from the result of the Floyd-Roy-Warshall
   * algorithm.
   * 
   * @param aGraph
   ...
   *         negative cycles
   */
  public static Object[] getWFIPath(mxAnalysisGraph aGraph, ArrayList<Object[][]> FWIresult,
      Object startVertex, Object targetVertex) throws StructuralException {
   ...
   * @throws StructuralException
   */
  private static ArrayList<Object> getWFIPathRec(mxAnalysisGraph aGraph, Object[][] paths,
      Object startVertex, Object targetVertex, ArrayList<Object> currPath, mxCostFunction cf,
    ...
    if (parents[tIndex] != null) {
      ...
      if (mxGraphStructure.areConnected(aGraph, startVertex, targetVertex)
