...
public class mxGraphGenerator {
    ...
    for (int j = 0; j < numRows; j++) {
      for (int i = 0; i < numColumns; i++) {
        ...
        if (i > 0) {
          ...
          graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[vertexCount - 1],
              currVertex);
        }
   ...
   * @param numColumns - number of columns in the grid graph
   */
  public void setGridGraphSpacing(mxAnalysisGraph aGraph, double xSpacing, double ySpacing,
   ...
   * @return a bipartite graph with group 1 containing <b>numVerticesGroup1</b> vertices and group 2
   *         containing <b>numVerticesGroup2</b>
   */
  public void getBipartiteGraph(mxAnalysisGraph aGraph, int numVerticesGroup1,
    ...
    for (int j = 0; j < numVerticesGroup2; j++) {
      ...
      int edgeNum = aGraph.getOpposites(aGraph.getEdges(currVertex, null, true, true, false, true),
  ...
  /**
   * Sets the physical spacing between vertices in a bipartite graph. This works for now only for a
   * graph generated with mxGraphCreator.getBipartiteGraph() only after creating the graph
   * 
   * @param aGraph
   * @param numVerticesGroup1 - number of vertices in group 1
   * @param numVerticesGroup2 - number of vertices in group 2
   ...
   */
  public void setBipartiteGraphSpacing(mxAnalysisGraph aGraph, int numVerticesGroup1,
      int numVerticesGroup2, double vertexSpacing, double groupSpacing) {
    if (numVerticesGroup1 < 0 || numVerticesGroup2 < 0) {
      throw new IllegalArgumentException();
    }
   ...
   *         containing <b>numVerticesGroup2</b>
   */
  public void getCompleteBipartiteGraph(mxAnalysisGraph aGraph, int numVerticesGroup1,
      int numVerticesGroup2) {
    if (numVerticesGroup1 < 0 || numVerticesGroup2 < 0) {
      throw new IllegalArgumentException();
    }
   ...
   * @param xDim
   * @param yDim
   * @return a knight graph of size <b>xDim</b> x <b>yDim</b> Note that the minimum size is 3x3
   */
  public void getKnightGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
    ...
    for (int i = 0; i < (xDim * yDim); i++) {
      ...
      Object[] neighborMoves =
          getKnightMoveVertexes(aGraph, xDim, yDim, currCoords[0], currCoords[1]);
   ...
   * @return a list of ALL vertexes which would be valid moves from the current position, regardless
   *         if they were visited or not Note that both dimensions and both coordinates must be
   *         positive
   */
  public Object[] getKnightMoveVertexes(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord,
      int yCoord) {
   ...
   * @return int[x,y] where x and y are the coordinates in the grid or chess-board Note that both
   ...
   * @param xCoord x coordinate on the chess-board, coordinate starts from 1
   * @param yCoord y coordinate on the chess-board, coordinate starts from 1
   * @return vertex on the desired coordinates. Note that both dimensions and both coordinates must
   *         be positive
   */
   ...
   * @param xDim
   * @param yDim
   * @param weights Return a king graph of size <b>xDim</b> x <b>yDim</b> Note that the minimum size
   *        is 4x4
   */
  public void getKingGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
    ...
    for (int i = 0; i < (xDim * yDim); i++) {
      currCoords = getVertexGridCoords(xDim, yDim, i);
      Object[] neighborMoves =
          getKingMoveVertexes(aGraph, xDim, yDim, currCoords[0], currCoords[1]);
   ...
   * @return list of all possible moves of a king from the specified position Note that both
   *         dimensions and both coordinates must be positive
   */
  public Object[] getKingMoveVertexes(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord,
      int yCoord) {
  ...
  /**
   * Sets the physical spacing between vertices in a path graph. This works for now only for a graph
   * generated with mxGraphCreator.getPathGraph() only after creating the graph
   * 
   * @param aGraph
   * @param spacing
   */
  ...
  /**
   * @param aGraph
   * @param numVertices Returns a star graph Note that minimum vertex number is 4
   */
  public void getStarGraph(mxAnalysisGraph aGraph, int numVertices) {
    ...
    for (int i = 0; i < numVertexesInPerimeter; i++) {
      graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[numVertexesInPerimeter],
          vertices[i]);
    }
  };

  /**
   * Sets the physical size of a star graph. This works for now only for a graph generated with
   * mxGraphCreator.getStarGraph() and getWheelGraph()
   * 
   * @param aGraph
   * @param graphSize
   */
  ...
  public void getWheelGraph(mxAnalysisGraph aGraph, int numVertices) {
    ...
    for (int i = 0; i < numVerticesInPerimeter; i++) {
      graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[numVerticesInPerimeter],
          vertices[i]);
   ...
   * @param branchSize number of vertices in a single branch (minimum >= 2) Returns a friendship
   *        windmill graph (aka Dutch windmill)
   */
  public void getFriendshipWindmillGraph(mxAnalysisGraph aGraph, int numBranches, int branchSize) {
    ...
    vertices[numVertices - 1] =
        graph.insertVertex(parent, null, new Integer(numVertices - 1).toString(), 0, 0, 25, 25);
   ...
   * @param aGraph
   * @param numBranches - number of branches (minimum >= 2)
   * @param branchSize - number of vertices in a single branch (minimum >= 2) Returns a windmill
   *        graph
   */
  ...
  /**
   * Sets the layout of a windmill graph. Use this method only for graphs generated with
   * mxGraphGenerator.getWindmillGraph() and getFriendshitWindmillGraph()
   * 
   * @param aGraph
   * @param numBranches
  ...
  /**
   * A helper function that calculates the ring size for a windmill graph, based on the index of a
   * vertex in a brach and branch size. - for internal use
   * 
   ...
   * @param allowMultipleEdges if true, there will be a chance that multiple edges will be generated
   *        (multiple edges between the same two vertices)
   * @param forceConnected if true the resulting graph will be always connected, but this may alter
   *        <b>numEdges</b>
   */
  public void getSimpleRandomGraph(mxAnalysisGraph aGraph, int numNodes, int numEdges,
      boolean allowSelfLoops, boolean allowMultipleEdges, boolean forceConnected) {
    mxGraph graph = aGraph.getGraph();
    Object parent = graph.getDefaultParent();
      ...
      do {
        ...
        if (!allowSelfLoops && startVertex.equals(endVertex)) {
          goodPair = false;
            ...
            && mxGraphStructure.areConnected(aGraph, startVertex, endVertex)) {
          goodPair = false;
        }
      } while (!goodPair);
  ...
  /**
   * Creates a new edge value based on graph properties in mxAnalysisGraph. Used mostly when
   * creating new edges during graph generation.
   * 
   * @param aGraph
   * @return
   */
  ...
  /**
   * @param graph
   * @param weighted if true, the edges will be weighted, otherwise all will have default value
   *        (1.0)
   * @param minWeight minimum edge weight if weighted
   * @param maxWeight maximum edge weight if weighted
   * @return a generator function
   */
  public static mxGeneratorFunction getGeneratorFunction(mxGraph graph, boolean weighted,
   ...
   * @param forceSimple if true, a non-simple graph is made simple Calculates one spanning tree of
   *        graph, which doesn't have to be but can be minimal (this is faster than minimal spanning
   *        tree, so if you need any spanning tree, use this one) Self loops and multiple edges are
   *        automatically removed! Also, unconnected graphs are made connected!
  ...
  // TODO make a double check to avoid unnecessary cases of throwing an exception (if the algorithm
  // can't work out a solution, try a mirrored strategy)
  /**
   * @param aGraph
   * @param xDim x dimension of the chessboard
   * @param yDim y dimension of the chessboard
   * @param startVertexValue vertex where the tour will start
   * @throws StructuralException not all size combinations are allowed, see wikipedia for a more
  ...
  public void getKnightTour(mxAnalysisGraph aGraph, int xDim, int yDim, int startVertexValue)
    ...
    Object nextMove =
        getNextKnightMove(aGraph, xDim, yDim, currCoords[0], currCoords[1], resultPath);
   ...
   * @return
   */
  private Object getNextKnightMove(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord,
      int yCoord, ArrayList<Object> resultPath) {
    ...
    for (int i = 0; i < possibleMoves.length; i++) {
      ...
      int currMoveNum =
          getPossibleKnightMoveCount(aGraph, xDim, yDim, currCoords[0], currCoords[1]);
   ...
   * @return
   */
  private int getPossibleKnightMoveCount(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord,
      int yCoord) {
    // check all possible 8 locations
