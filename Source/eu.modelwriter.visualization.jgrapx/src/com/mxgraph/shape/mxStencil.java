...
/**
 * Implements a stencil for the given XML definition. This class implements the mxGraph stencil
 * schema.
 */
public class mxStencil implements mxIShape {
  ...
  protected void drawShadow(mxGraphicsCanvas2D canvas, mxCellState state, double rotation,
      boolean flipH, boolean flipV, mxRectangle bounds, double alpha, boolean filled) {
    ...
    mxPoint offset = mxUtils.getRotatedPoint(
        new mxPoint(mxConstants.SHADOW_OFFSETX, mxConstants.SHADOW_OFFSETY), cos, sin);
   ...
   */
  public boolean drawShape(mxGraphicsCanvas2D canvas, mxCellState state, mxRectangle bounds,
      boolean background) {
    ...
    if (elt != null) {
      ...
      double sw = strokewidth.equals("inherit")
          ? mxUtils.getDouble(state.getStyle(), mxConstants.STYLE_STROKEWIDTH, 1)
  ...
  /**
   * Returns a rectangle that contains the offset in x and y and the horizontal and vertical scale
   * in width and height used to draw this shape inside the given rectangle.
   */
  protected mxRectangle computeAspect(mxCellState state, mxRectangle bounds, String direction) {
    ...
    boolean inverse =
        (direction != null && (direction.equals("north") || direction.equals("south")));
   ...
   */
  protected void drawElement(mxGraphicsCanvas2D canvas, mxCellState state, Element node,
      mxRectangle aspect) {
    ...
    if (name.equals("save")) {
      ...
      canvas.quadTo(x0 + getDouble(node, "x1") * sx, y0 + getDouble(node, "y1") * sy, lastMoveX,
          lastMoveY);
      ...
      double[] curves = mxUtils.arcToCurves(this.lastMoveX, this.lastMoveY, r1, r2, angle,
          largeArcFlag, sweepFlag, x, y);

      for (int i = 0; i < curves.length; i += 6) {
        canvas.curveTo(curves[i], curves[i + 1], curves[i + 2], curves[i + 3], curves[i + 4],
            curves[i + 5]);
      ...
      canvas.rect(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy,
          getDouble(node, "w") * sx, getDouble(node, "h") * sy);
      ...
      canvas.roundrect(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy,
          getDouble(node, "w") * sx, getDouble(node, "h") * sy, r, r);
    } else if (name.equals("ellipse")) {
      canvas.ellipse(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy,
          getDouble(node, "w") * sx, getDouble(node, "h") * sy);
    } else if (name.equals("image")) {
      String src = evaluateAttribute(node, "src", state);

      canvas.image(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy,
          getDouble(node, "w") * sx, getDouble(node, "h") * sy, src, false,
          getString(node, "flipH", "0").equals("1"), getString(node, "flipV", "0").equals("1"));
      ...
      canvas.text(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, 0, 0, str,
          node.getAttribute("align"), node.getAttribute("valign"), false, "", null, false, rotation,
          null);
  ...
  /**
   * Gets the attribute for the given name from the given node. If the attribute does not exist then
   * the text content of the node is evaluated and if it is a function it is invoked with <state> as
   * the only argument and the return value is used as the attribute value to be returned.
   */
