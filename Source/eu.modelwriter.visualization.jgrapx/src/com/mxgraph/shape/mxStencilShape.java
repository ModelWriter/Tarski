...
public class mxStencilShape extends mxBasicShape {
  ...
  /**
   * Transform cached to save instance created. Used to scale the internal path of shapes where
   * possible
   */
   ...
   * 
   */
  public void paintNode(mxGraphics2DCanvas canvas, mxCellState state, svgShape shape,
   ...
   */
  protected void transformShape(Shape shape, double transX, double transY, double widthRatio,
      double heightRatio) {
    if (shape instanceof Rectangle2D) {
      Rectangle2D rect = (Rectangle2D) shape;
      if (transX != 0 || transY != 0) {
        rect.setFrame(rect.getX() + transX, rect.getY() + transY, rect.getWidth(),
            rect.getHeight());
      }

      if (widthRatio != 1 || heightRatio != 1) {
        rect.setFrame(rect.getX() * widthRatio, rect.getY() * heightRatio,
            rect.getWidth() * widthRatio, rect.getHeight() * heightRatio);
      }
      ...
      if (widthRatio != 1 || heightRatio != 1) {
        line.setLine(line.getX1() * widthRatio, line.getY1() * heightRatio,
            line.getX2() * widthRatio, line.getY2() * heightRatio);
      }
      ...
      if (isRectangle(root.getNodeName())) {
        ...
        try {
          ...
          if (rx > 0 || ry > 0) {
            ...
            rectShape =
                new svgShape(new RoundRectangle2D.Double(x, y, width, height, rx, ry), styleMap);
          } else {
