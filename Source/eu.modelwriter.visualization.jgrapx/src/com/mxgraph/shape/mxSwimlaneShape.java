...
public class mxSwimlaneShape extends mxBasicShape {
  ...
  protected double getTitleSize(mxGraphics2DCanvas canvas, mxCellState state) {
    return Math.max(0, mxUtils.getFloat(state.getStyle(), mxConstants.STYLE_STARTSIZE,
  ...
  public void paintShape(mxGraphics2DCanvas canvas, mxCellState state) {
    double start = getTitleSize(canvas, state);
    String fill =
        mxUtils.getString(state.getStyle(), mxConstants.STYLE_SWIMLANE_FILLCOLOR, mxConstants.NONE);
    ...
    String sep =
        mxUtils.getString(state.getStyle(), mxConstants.STYLE_SEPARATORCOLOR, mxConstants.NONE);
   ...
   * Helper method to configure the given wrapper canvas.
   */
  protected mxGraphicsCanvas2D configureCanvas(mxGraphics2DCanvas canvas, mxCellState state,
    ...
    if (!mxConstants.NONE.equals(fill) && !mxConstants.NONE.equals(gradient)) {
      ...
      c.setGradient(fill, gradient, b.getX(), b.getY(), b.getWidth(), b.getHeight(),
          mxUtils.getString(state.getStyle(), mxConstants.STYLE_GRADIENT_DIRECTION,
   ...
   * 
   */
  protected void paintSwimlane(mxGraphics2DCanvas canvas, mxCellState state, double start,
      String fill, boolean swimlaneLine) {
    mxGraphicsCanvas2D c =
        configureCanvas(canvas, state, new mxGraphicsCanvas2D(canvas.getGraphics()));
   ...
   */
  protected void paintRoundedSwimlane(mxGraphics2DCanvas canvas, mxCellState state, double start,
      double r, String fill, boolean swimlaneLine) {
    mxGraphicsCanvas2D c =
        configureCanvas(canvas, state, new mxGraphicsCanvas2D(canvas.getGraphics()));
    double w = state.getWidth();
    double h = state.getHeight();
   ...
   * Paints the swimlane vertex shape.
   */
  protected void paintSeparator(mxGraphics2DCanvas canvas, mxCellState state, double start,
      String color) {
