...
public class mxCurveLabelShape implements mxITextShape {
    ...
    if (!label.equals(lastValue) || !font.equals(lastFont)) {
      ...
      if (bidiRequired) {
        ...
        for (int i = 0; i < bidi.getRunCount(); i++) {
          ...
          rtlGlyphVectors[i] = font.layoutGlyphVector(mxCurveLabelShape.frc,
              labelSection.toCharArray(), 0, labelSection.length(), Font.LAYOUT_RIGHT_TO_LEFT);
        }
  ...
  public boolean intersectsRect(Rectangle rect) {
    ...
    if ((labelBounds != null && (!labelBounds.getRectangle().intersects(rect)))
        || labelGlyphs == null) {
      return false;
    }
  ...
  /**
   * Utility class to describe the characteristics of each glyph of a branch branch label. Each
   * instance represents one glyph
   *
   */
  public class LabelGlyphCache {
    /**
     * Cache of the bounds of the individual element of the label of this edge. Note that these are
     * the unrotated values used to determine the width of each glyph.
     */
