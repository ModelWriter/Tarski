...
public class mxGraphTransferHandler extends TransferHandler {
  ...
  /**
   * Specifies the background color of the transfer image. If no color is given here then the
   * background color of the enclosing graph component is used. Default is Color.WHITE.
   */
  ...
  /**
   * Sets the value for the initialImportCount. Default is 1. Updated in exportDone to contain 0
   * after a cut and 1 after a copy.
   */
  ...
  /**
   * Specifies if a transfer image should be created for the transferable. Default is
   * DEFAULT_TRANSFER_IMAGE.
   */
    ...
    if (c instanceof mxGraphComponent) {
      ...
      if (!graph.isSelectionEmpty()) {
        ...
        if (originalCells.length > 0) {
          ImageIcon icon = (transferImageEnabled)
              ? createTransferableImage(graphComponent, originalCells) : null;
   ...
   */
  public mxGraphTransferable createGraphTransferable(mxGraphComponent graphComponent,
      Object[] cells, mxRectangle bounds, ImageIcon icon) {
  ...
  /**
   * Checks if the mxGraphTransferable data flavour is supported and calls importGraphTransferable
   * if possible.
   */
  ...
  /**
   * Gets a drop target using getDropTarget and imports the cells using mxGraph.splitEdge or
   * mxGraphComponent.importCells depending on the drop target and the return values of
   * mxGraph.isSplitEnabled and mxGraph.isSplitTarget. Selects and returns the cells that have been
   * imported.
   */
  protected Object[] importCells(mxGraphComponent graphComponent, mxGraphTransferable gt, double dx,
      double dy) {
