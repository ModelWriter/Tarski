...
public class mxEdgeHandler extends mxCellHandler {
  ...
  protected mxCellMarker marker = new mxCellMarker(graphComponent) {
    ...
    protected boolean isValidState(mxCellState state) {
      ...
      mxCellState other =
          view.getTerminalPort(state, view.getState(model.getTerminal(edge, !isSource)), !isSource);
  ...
  protected Color getHandleFillColor(int index) {
    ...
    if (source || isTarget(index)) {
      ...
      if (terminal == null
          && !graphComponent.getGraph().isTerminalPointMovable(state.getCell(), source)) {
        ...
        return (graphComponent.getGraph().isCellDisconnectable(state.getCell(), terminal, source))
            ? mxSwingConstants.CONNECT_HANDLE_FILLCOLOR : mxSwingConstants.LOCKED_HANDLE_FILLCOLOR;
      }
    }
  ...
  public void mouseDragged(MouseEvent e) {
    if (!e.isConsumed() && first != null) {
      ...
      if (isLabel(index)) {
        ...
        mxConnectionConstraint sourceConstraint =
            graphComponent.getGraph().getConnectionConstraint(clone, sourceState, true);
        mxConnectionConstraint targetConstraint =
            graphComponent.getGraph().getConnectionConstraint(clone, targetState, false);
         ...
         * 
         * if (isSource) { sourceConstraint = constraint; } else if (isTarget) { targetConstraint =
         * constraint; }
         */
      ...
      if (!preview.isVisible()
          && graphComponent.isSignificant(e.getX() - first.x, e.getY() - first.y)) {
    ...
    if (!e.isConsumed() && first != null) {
      ...
      if (graphComponent.isSignificant(dx, dy)) {
        if (error != null) {
          ...
          movePoint(state.getCell(), index,
              convertPoint(new mxPoint(e.getPoint()), gridEnabledEvent));
        }
  ...
  protected void moveLabelTo(mxCellState edgeState, double x, double y) {
    ...
    if (geometry != null) {
      ...
      geometry.setOffset(
          new mxPoint(Math.round((x - pt.getX()) / scale), Math.round((y - pt.getY()) / scale)));
