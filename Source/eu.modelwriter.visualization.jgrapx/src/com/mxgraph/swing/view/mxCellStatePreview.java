...
public class mxCellStatePreview {
  ...
  /**
   * Constructs a new state preview. The paint handler to invoke the paint method must be installed
   * elsewhere.
   */
   ...
   * 
   */
  public mxPoint moveState(mxCellState state, double dx, double dy, boolean add,
   ...
   *
   */
  protected mxRectangle revalidateState(mxCellState parentState, mxCellState state, double dx,
      double dy) {
    mxRectangle dirty = null;
    ...
    if (state != null) {
      ...
      if ((dx != 0 || dy != 0) && geo != null && geo.isRelative() && model.isVertex(cell)
          && (parentState == null || model.isVertex(parentState.getCell())
              || deltas.get(state) != null)) {
        state.setX(state.getX() + dx);
        state.setY(state.getY() + dy);
      ...
      for (int i = 0; i < childCount; i++) {
        mxRectangle tmp =
            revalidateState(state, graph.getView().getState(model.getChildAt(cell, i)), dx, dy);
