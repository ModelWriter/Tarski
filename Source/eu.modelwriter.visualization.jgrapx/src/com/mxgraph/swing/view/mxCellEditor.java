...
/**
 * To control this editor, use mxGraph.invokesStopCellEditing, mxGraph. enterStopsCellEditing and
...
public class mxCellEditor implements mxICellEditor {
  ...
  /**
   * Defines the minimum scale to be used for the editor. Set this to 0 if the font size in the
   * editor
   */
  ...
  /**
   * Specifies if the text content of the HTML body should be extracted before and after editing for
   * HTML markup. Default is true.
   */
  protected boolean extractHtmlBody = true;

  /**
   * Specifies if linefeeds should be replaced with BREAKS before editing, and BREAKS should be
   * replaced with linefeeds after editing. This value is ignored if extractHtmlBody is false.
   * Default is true.
   */
  ...
  protected boolean useLabelBounds(mxCellState state) {
    ...
    return ((geometry != null && geometry.getOffset() != null && !geometry.isRelative()
        && (geometry.getOffset().getX() != 0 || geometry.getOffset().getY() != 0))
