...
public class mxInteractiveCanvas extends mxGraphics2DCanvas {
    ...
    if (state != null) {
      ...
      if (pointCount > 0) {
        ...
        if (mxUtils.getString(state.getStyle(), mxConstants.STYLE_SHAPE, "")
            .equals(mxConstants.SHAPE_ARROW)) {
  ...
  /**
   * Returns true if the given point is inside the content area of the given swimlane. (The content
   * area of swimlanes is transparent to events.) This implementation does not check if the given
   * state is a swimlane, it is assumed that the caller has checked this before using this method.
   */
  public boolean hitSwimlaneContent(mxGraphComponent graphComponent, mxCellState swimlane, int x,
    ...
    if (swimlane != null) {
      int start =
          (int) Math
              .max(2,
                  Math.round(mxUtils.getInt(swimlane.getStyle(), mxConstants.STYLE_STARTSIZE,
