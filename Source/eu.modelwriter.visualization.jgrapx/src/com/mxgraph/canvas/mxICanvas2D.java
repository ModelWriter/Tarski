 ...
 * 
 * - Path rendering (move, line, quad, curve, arc) - Images, flip v/h, aspect, alpha (PNG, JPG, GIF)
 * - Linear gradients (in all four directions) - Transparency, fill and stroke - Rotation, flip v/h
 * - Font rendering - Dash patterns - Clipping by path (not just rectangle) - Alpha gradients (for
 * glass effect) - Encode result as image (PNG, JPG)
 */
public interface mxICanvas2D {
  ...
  /**
   * Rotates the canvas by the given angle around the given center. This method may add rendering
   * overhead and should be used with care.
   * 
  ...
  /**
   * Sets the stroke color. This should default to {@link mxConstants#NONE} if unset.
   * 
   ...
   * 
   * @param value Space separated list of floats representing the dash pattern. The value should be
   ...
   * @param w
   * @param h
   * @param direction Direction may be null. Use default value {@link mxConstants#DIRECTION_SOUTH}.
   */
  void setGradient(String color1, String color2, double x, double y, double w, double h,
   ...
   * @param flipV
   */
  void image(double x, double y, double w, double h, String src, boolean aspect, boolean flipH,
      boolean flipV);

  /**
   * Draws the given string. Possible values for format are empty string for plain text and html for
   * HTML markup.
   * 
   * @param x
   * @param y
  ...
  void text(double x, double y, double w, double h, String str, String align, String valign,
      boolean wrap, String format, String overflow, boolean clip, double rotation, String dir);
