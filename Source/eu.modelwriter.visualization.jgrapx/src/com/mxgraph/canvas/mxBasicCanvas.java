...
public abstract class mxBasicCanvas implements mxICanvas {
  ...
  /**
   * Defines the default value for the imageBasePath in all GDI canvases. Default is an empty
   * string.
   */
  public static String DEFAULT_IMAGEBASEPATH = "";

  /**
   * Defines the base path for images with relative paths. Trailing slash is required. Default value
   * is DEFAULT_IMAGEBASEPATH.
   */
  ...
  /**
   * Returns an image instance for the given URL. If the URL has been loaded before than an instance
   * of the same instance is returned as in the previous call.
   */
  ...
  /**
   * Gets the image path from the given style. If the path is relative (does not start with a slash)
   * then it is appended to the imageBasePath.
   */
