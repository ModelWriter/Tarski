...
/**
 * Used for exporting images. To render to an image from a given XML string, graph size and
 * background color, the following code is used:
 * 
 * <code>
 ...
 * </code>
 * 
 * Text rendering is available for plain text and HTML markup, the latter with optional word
 * wrapping. CSS support is limited to the following:
...
public class mxGraphicsCanvas2D implements mxICanvas2D {
  ...
  /**
   * Specifies the additional pixels when computing the text width for HTML labels. Default is 2.
   */
   ...
   * 
   */
  public void setGradient(String color1, String color2, double x, double y, double w, double h,
      String direction, double alpha1, double alpha2) {
   ...
   * 
   */
  public void image(double x, double y, double w, double h, String src, boolean aspect,
    ...
    if (src != null && w > 0 && h > 0) {
      ...
      if (img != null) {
        ...
        if (img != null) {
          drawImage(
              createImageGraphics(bounds.x, bounds.y, bounds.width, bounds.height, flipH, flipV),
   ...
   * 
   */
  protected final Rectangle getImageBounds(Image img, double x, double y, double w, double h,
      boolean aspect) {
   ...
   */
  protected final Graphics2D createImageGraphics(double x, double y, double w, double h,
      boolean flipH, boolean flipV) {
  ...
  protected String createHtmlDocument(String text, String align, String valign, int w, int h,
      boolean wrap, String overflow, boolean clip) {
    ...
    css.append("line-height:" + ((mxConstants.ABSOLUTE_LINE_HEIGHT)
        ? Math.round(state.fontSize * mxConstants.LINE_HEIGHT) + " pt" : mxConstants.LINE_HEIGHT)
        + ";");
  ...
  /**
   * Hook to return the renderer for HTML formatted text. This implementation returns the shared
   * instance of mxLighweightLabel.
   */
   ...
   */
  protected void htmlText(double x, double y, double w, double h, String str, String align,
      String valign, boolean wrap, String format, String overflow, boolean clip, double rotation) {
    x += state.dx;
    y += state.dy;
    ...
    if (textRenderer != null && rendererPane != null) {
      ...
      rendererPane.paintComponent(state.g, textRenderer, rendererPane, (int) Math.round(x),
          (int) Math.round(y), (int) Math.round(w), (int) Math.round(h), true);
   ...
   * Draws the given text.
   */
  public void plainText(double x, double y, double w, double h, String str, String align,
      String valign, boolean wrap, String format, String overflow, boolean clip, double rotation) {
   ...
   */
  protected final Graphics2D createTextGraphics(double x, double y, double w, double h,
      double rotation, boolean clip, String align, String valign) {
    Graphics2D g2 = state.g;
    updateFont();
  ...
  public void quadTo(double x1, double y1, double x2, double y2) {
    if (currentPath != null) {
      currentPath.quadTo((float) ((state.dx + x1) * state.scale),
          (float) ((state.dy + y1) * state.scale), (float) ((state.dx + x2) * state.scale),
  ...
  public void curveTo(double x1, double y1, double x2, double y2, double x3, double y3) {
    if (currentPath != null) {
      currentPath.curveTo((float) ((state.dx + x1) * state.scale),
          (float) ((state.dy + y1) * state.scale), (float) ((state.dx + x2) * state.scale),
          (float) ((state.dy + y2) * state.scale), (float) ((state.dx + x3) * state.scale),
  ...
  /**
   * Returns a font name for the given CSS values for font-family. This implementation returns the
   * first entry for comma-separated lists of entries.
   */
