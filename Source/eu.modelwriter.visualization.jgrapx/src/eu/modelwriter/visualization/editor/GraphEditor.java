...
public class GraphEditor extends JPanel {
	...
	protected void installListeners() {
		...
		this.graphComponent.getGraphControl().addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(final MouseEvent e) {
				...
				if (GraphEditor.this.onWhat != null) {
					...
					if (cell.isVertex()) {
						GraphEditor.this.oldCenterX = ((mxCell) GraphEditor.this.onWhat).getGeometry().getCenterX();
						GraphEditor.this.oldCenterY = ((mxCell) GraphEditor.this.onWhat).getGeometry().getCenterY();
					} else if (cell.isEdge()) {
						GraphEditor.this.controlPointOrder = EdgeUtil.getInstance(graph, graphComponent)
								.getControlPointOrder(cell, e.getX(), e.getY());
						if (controlPointOrder == -1) {
							return;
						}
						GraphEditor.this.layerOfControlPoint = EdgeUtil.getInstance(graph, graphComponent).layer(cell,
								controlPointOrder);
						mxPoint point = EdgeUtil.getInstance(graph, graphComponent).getControlPoint(cell,
								controlPointOrder);
						if (point == null) {
							return;
						}
		...
		this.graphComponent.getGraphControl().addMouseMotionListener(new MouseMotionListener() {
			private void clear() {
				...
				GraphEditor.this.graph.setCellStyles(mxConstants.STYLE_FILLCOLOR, null, GraphEditor.this.objs);
			...
			public void mouseDragged(final MouseEvent e) {
				GraphEditor.this.newCenterX = (int) (GraphEditor.this.oldCenterX
						+ (e.getX() - GraphEditor.this.oldMouseX));
				GraphEditor.this.newCenterY = (int) (GraphEditor.this.oldCenterY
						+ (e.getY() - GraphEditor.this.oldMouseY));
				...
				if (GraphEditor.this.onWhat != null) {
					...
					if (cell.isVertex()) {
						...
						GraphEditor.this.reverses = EdgeUtil
								.getInstance(GraphEditor.this.graph, GraphEditor.this.graphComponent).getReverseEdges(
										EdgeUtil.getInstance(GraphEditor.this.graph, GraphEditor.this.graphComponent)
												.getEdgeName(cell));
						GraphEditor.this.sameEdges = EdgeUtil
								.getInstance(GraphEditor.this.graph, GraphEditor.this.graphComponent).getSameEdges(
										EdgeUtil.getInstance(GraphEditor.this.graph, GraphEditor.this.graphComponent)
												.getEdgeName(cell));
	...
	protected void showGraphPopupMenu(final MouseEvent e) {
		final Point pt = SwingUtilities.convertPoint(e.getComponent(), e.getPoint(), this.graphComponent);
	...
	private void tweakCell(final MouseEvent e) {
		if (GraphEditor.this.onWhat != null) {
			...
			if (cell.isVertex()) {
				...
				final mxPoint point = EdgeUtil.getInstance(this.graph, this.graphComponent).getControlPoint(cell,
						this.controlPointOrder);
				if (point == null) {
					return;
				}
				NodeUtil.getInstance(this.graph, this.graphComponent).tweakControlPoint(cell, this.controlPointOrder,
						(int) this.newCenterY);
				point.setX(e.getX());
				point.setY(e.getY());
				if (point.getY() < 0) {
				...
				final int sum = GraphUtil.getInstance(graph, graphComponent).runLayerMinY()[layerOfControlPoint]
						+ GraphUtil.getInstance(graph, graphComponent).layerPH()[layerOfControlPoint] / 2;
				if (newCenterY < sum) {
					mxPoint controlPoint = EdgeUtil.getInstance(graph, graphComponent).getControlPoint((mxCell) onWhat,
							controlPointOrder);
