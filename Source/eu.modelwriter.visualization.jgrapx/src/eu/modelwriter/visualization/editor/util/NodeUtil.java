...
public class NodeUtil {
	...
	public int layer(final mxCell cell) {
		return cell.getAttribute(GraphUtil.LAYER) == null ? -1 : Integer.valueOf(cell.getAttribute(GraphUtil.LAYER));
	}
	...
	public int reserved(final mxCell cell) {
		...
		for (int i = 0; i < selfs.size(); i++) {
			...
			reserved = reserved + (int) NodeUtil.graph.getView().getState(selfs.get(i - 1)).getLabelBounds().getWidth()
					+ GraphUtil.selfLoopGL + GraphUtil.selfLoopGR;
		}
		if (reserved > 0) {
			reserved = reserved
					+ (int) NodeUtil.graph.getView().getState(selfs.get(selfs.size() - 1)).getLabelBounds().getWidth()
					+ GraphUtil.selfLoopGL + GraphUtil.selfLoopGR;
		}
		return reserved;
	}
	...
	public void setX(final mxCell cell, final int newCenterX) {
		final int oldCenterX = centerX(cell);
		NodeUtil.graph.moveCells(new Object[] { cell }, newCenterX - oldCenterX, 0);
		final int lefOfCell = newCenterX - this.side(cell);
		if (lefOfCell < 0) {
			NodeUtil.graph.moveCells(NodeUtil.graphUtilInstance.getNodes().toArray(), Math.abs(lefOfCell), 0);
	...
	/**
	 * calculates size from center of vertex to vertical edge of vertex with
	 * padding.
	 * 
	 * @param cell
	 *            must be a vertex
	...
	private void swapLeft(final mxCell cell, final List<mxCell> peers, int orderInLayer, final int newCenterX) {
		...
		while (true) {
			...
			orderInLayer--;
			this.shiftRight(other, peers, orderInLayer + 1, newCenterX + side + this.reserved(cell) + otherSide);
		}
	}

	/** Helper method that swaps a node towards the right. */
	private void swapRight(final mxCell cell, final List<mxCell> peers, int orderInLayer, final int newCenterX) {
		final int side = this.side(cell);
		...
		while (true) {
			...
			NodeUtil.graphUtilInstance.swapNodes(this.layer(cell), orderInLayer, orderInLayer + 1);
			orderInLayer++;
			this.shiftLeft(other, peers, orderInLayer - 1, newCenterX - side - this.reserved(other) - otherSide);
		}
	}
	...
	/**
	 * calculates size from center of vertex to horizontal edge of vertex with
	 * padding.
	 * 
	 * @param cell
	 *            must be a vertex
	...
	public int x(final mxCell cell) {
		return centerX(cell) - side(cell);
	}

	public int y(final mxCell cell) {
		return centerY(cell) - updown(cell);
	}
}
