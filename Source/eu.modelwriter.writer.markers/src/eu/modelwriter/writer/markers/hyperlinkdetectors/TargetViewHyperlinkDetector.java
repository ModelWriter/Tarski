...
public class TargetViewHyperlinkDetector extends AbstractHyperlinkDetector implements IHyperlinkDetector {
		...
		for (IMarker iMarker : markedList) {
			try {
				...
				IRegion targetRegion = new Region((int) iMarker.getAttribute(IMarker.CHAR_START),
						(int) iMarker.getAttribute(IMarker.CHAR_END) - (int) iMarker.getAttribute(IMarker.CHAR_START));
